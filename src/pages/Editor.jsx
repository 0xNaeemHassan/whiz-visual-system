...
import { createTelemetry } from '../telemetry/instrumentation';
import { TELEMETRY_EVENTS } from '../telemetry/schema';
...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const track = useMemo(
    () => createTelemetry({ frameId, layout: selectedFrame?.layout }),
    [frameId, selectedFrame?.layout],
  );
  ...
  const doSave=()=>{const id=`s_${Date.now()}`;const n=saveName.trim()||`${content.topicTag} \u2014 ${new Date().toLocaleDateString()}`;setSaves(p=>[...p,{id,title:n,...buildSave()}]);track(TELEMETRY_EVENTS.SAVE,{saveId:id,saveName:n});setShowSaveModal(false);setSaveName('');showToast(`Saved "${n}"`);};
  const loadSave=s=>{setFrameId(s.frameId);setTheme(s.theme);resetContent(s.content);s.overrides&&setOverrides(s.overrides);s.aspectRatio&&setAspectRatio(s.aspectRatio);s.bgGradient&&setBgGradient(s.bgGradient);s.patternOverlay&&setPatternOverlay(s.patternOverlay);track(TELEMETRY_EVENTS.LOAD,{source:'manual',saveId:s.id},{frameId:s.frameId,layout:FRAMES.find(f=>f.id===s.frameId)?.layout||selectedFrame?.layout});setShowLoadModal(false);showToast(`Loaded`);};
  ...
  const exportWebP=async()=>{
    ...
    try{
      ...
      c.style.cssText=el.style.cssText;c.style.transform='none';c.style.position='fixed';c.style.left='0';c.style.top='0';
      document.body.appendChild(c);
      let cv;
      try{
        cv=await h2c(c,{scale:2,useCORS:true,allowTaint:true,width:aspectRatio.w,height:aspectRatio.h,backgroundColor:overrides.frameBg||theme.base,logging:false});
      }finally{if(document.body.contains(c))document.body.removeChild(c);}
      await new Promise((res,rej)=>cv.toBlob(b=>{
        ...
        track(TELEMETRY_EVENTS.EXPORT_SUCCESS,{format:'webp',strictMode});showToast('WebP exported');res();
      },'image/webp',0.92));
