  ...
  return(
    <div className="editor-wrap" ref={editorRootRef}>
      ...
      <div className={`editor-center ${mobileTab==='preview'?'mob-active':''}`} ref={centerRef}>
        <div className="frame-scale-wrap" style={FRAME_SCALE_STYLE(zoom)}><WhizFrame trustLevel={trustLevel} frameRef={frameRef} frame={selectedFrame} theme={theme} content={content} editMode={editMode} selectedEl={selectedEl} onSelectEl={k=>{setSelectedEl(k);k&&setRightTab('design');}} styleOverrides={overrides} showGrid={showGrid} aspectRatio={aspectRatio} uploadedImages={uploadedImages} bgGradient={bgGradient} patternOverlay={patternOverlay} strictWhizMode={strictWhizMode} whizEffects={whizEffects}
            fontPairing={activeFontPairing}/></div>
        <div className="zoom-bar"><IconButton className="zoom-btn" label="Zoom out" onClick={()=>setZoom(z=>Math.max(0.1,+(z-0.05).toFixed(2)))}>−</IconButton><span className="zoom-pct">{Math.round(zoom*100)}%</span><IconButton className="zoom-btn" label="Zoom in" onClick={()=>setZoom(z=>Math.min(1,+(z+0.05).toFixed(2)))}>+</IconButton><IconButton className="zoom-btn" label="Fit frame to viewport" onClick={updateZoom} style={ZOOM_FIT_STYLE}>⊡</IconButton><div style={ZOOM_DIVIDER_STYLE}/><IconButton className={`zoom-btn ${showGrid?'active':''}`} label={showGrid ? 'Hide grid overlay' : 'Show grid overlay'} onClick={()=>setShowGrid(g=>!g)} style={showGrid ? { color: 'var(--theme-accent)' } : undefined}>▦</IconButton><IconButton className={`zoom-btn ${editMode?'active':''}`} label={editMode ? 'Disable edit mode' : 'Enable edit mode'} onClick={()=>{setEditMode(m=>!m);editMode&&setSelectedEl(null);}} style={editMode ? { color: 'var(--theme-accent)' } : undefined}>✎</IconButton><div style={ZOOM_DIVIDER_STYLE}/><IconButton className="zoom-btn" label="Undo" onClick={()=>handleUndo(mobileTab==='preview'?'mobile':'desktop')} disabled={!canUndo} style={{opacity:canUndo?1:0.3}}>↶</IconButton><IconButton className="zoom-btn" label="Redo" onClick={()=>handleRedo(mobileTab==='preview'?'mobile':'desktop')} disabled={!canRedo} style={{opacity:canRedo?1:0.3}}>↷</IconButton></div>
        <div style={frameMetaStyle}>{String(frameId).padStart(2,'0')} — {selectedFrame.name} · {aspectRatio.w}×{aspectRatio.h}</div>
        <div style={trustBadgeStyle}>Trust: {trustLevel}</div>
        <div style={exportActionsStyle}><button className="btn btn-ghost btn-sm" onClick={exportJSON} style={EXPORT_ACTION_BUTTON_STYLE}>JSON</button><button className="btn btn-ghost btn-sm" onClick={exportManifest} style={EXPORT_ACTION_BUTTON_STYLE}>Manifest</button><button className="btn btn-secondary btn-sm" onClick={launchCorrectionFlow} style={EXPORT_ACTION_BUTTON_STYLE}>Correction</button><button className="btn btn-secondary btn-sm" onClick={exportHTML} style={EXPORT_ACTION_BUTTON_STYLE}>HTML</button><button className="btn btn-secondary btn-sm" onClick={applyStrictPolish} style={EXPORT_ACTION_BUTTON_STYLE}>Polish</button><button className="btn btn-primary btn-sm" data-format="png" onClick={exportPNG} disabled={exporting} style={EXPORT_ACTION_BUTTON_STYLE}>{exporting?'⟳':'↓'} PNG</button></div>
        <div style={readinessStyle}>Readiness: {readinessSummary.ready?'Ready to publish':`${readinessSummary.failed}/${readinessSummary.total} checks failing`} {readinessSummary.blocking>0?`· ${readinessSummary.blocking} blocking`:''}</div>
        {editMode&&<div style={editHintStyle}>{selectedEl?`Selected: ${selectedEl}`:'Click any element'}</div>}
      </div>
      {/* RIGHT */}
