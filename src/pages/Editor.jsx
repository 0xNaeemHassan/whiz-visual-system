  ...
  const exportWebP=async()=>{if(!validateExportTypography())return;
    ...
    try{setDatasetSnapshotRecord({snapshotId:currentDatasetSnapshot.snapshotId,hash:currentDatasetSnapshot.hash,schemaVersion:currentDatasetSnapshot.schemaVersion,savedAt:Date.now()});
      ...
      await new Promise((res)=>{
        const optimizedWebp=optimizeCanvasForExport(cv,{preferredType:'image/webp',fallbackType:'image/jpeg',qualityTargets:[0.92,0.88,0.84,0.8]});
        const a=document.createElement('a');
        const baseName=normalizedContent.slug||'whiz_export';const imageFileName=`${baseName}.webp`;a.href=optimizedWebp.dataUrl;a.download=imageFileName;a.click();
        const summary=buildExportSummary(normalizedContent);exportSummarySidecars(baseName,summary,imageFileName);
        track(TELEMETRY_EVENTS.EXPORT_SUCCESS,{format:'webp',strictMode,taxonomyAutoCorrected,exportOptimization:optimizedWebp.qualityTrail});showToast(`WebP exported + summary sidecars${usedFallback?' (DOM fallback)':''}`);res();
      });
