...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const exportSummarySidecars=(baseName,summary,imageFileName)=>{const overrideEvents=activityLog.filter((entry)=>entry.outcome==='override');const payload={...summary,imageFileName,exportMetadata:{overrideEventCount:overrideEvents.length,overrideEvents}};downloadFile(`${baseName}.summary.json`,new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));downloadFile(`${baseName}.summary.txt`,new Blob([buildSummaryText(payload)],{type:'text/plain'}));downloadFile(`${baseName}.manifest.signoff.json`,new Blob([JSON.stringify({reviewState,signoffRecord,savedAt:new Date().toISOString(),overrideEventCount:overrideEvents.length},null,2)],{type:'application/json'}));};
