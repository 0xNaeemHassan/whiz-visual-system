...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const buildExportSummary=(normalizedContent,exportContract)=>generateExportSummary({frame:activeFrame,content:{...content,...normalizedContent},complianceIssues,validationWarnings:editorValidation.warnings,exportContract});
  ...
  const exportWebP=async()=>{if(!validateExportTypography())return;
    ...
    try{setDatasetSnapshotRecord({snapshotId:currentDatasetSnapshot.snapshotId,hash:currentDatasetSnapshot.hash,schemaVersion:currentDatasetSnapshot.schemaVersion,savedAt:Date.now()});
      ...
      const resolvedPreset=resolveExportContractPreset({presetId:exportPresetId,baseDimensions:{width:aspectRatio.w,height:aspectRatio.h}});const {canvas:cv,contract,usedFallback}=await exportFrame({contractInput:{format:'webp',dimensions:resolvedPreset.dimensions,quality:resolvedPreset.quality,background:overrides.frameBg||theme.base,citationMode:resolvedPreset.citationMode,version:'1.0.0',presetId:resolvedPreset.presetId,effectsPolicy:resolvedPreset.effectsPolicy},sceneModel,sceneRenderer:renderSceneToCanvas,domFallbackRenderer:(contract)=>renderDomSnapshotToCanvas(frameRef.current,{width:contract.dimensions.width,height:contract.dimensions.height,backgroundColor:contract.background})});
      await new Promise((res,rej)=>cv.toBlob(b=>{
        ...
        const summary=buildExportSummary(normalizedContent,contract);exportSummarySidecars(baseName,summary,imageFileName);
