...
const normalizeTruncationPolicy = (policy) => {
  const base = policy && typeof policy === 'object' ? policy : {};
  const strategy = TRUNCATION_STRATEGIES.includes(base?.global?.strategy) ? base.global.strategy : DEFAULT_TRUNCATION_POLICY.global.strategy;
  const maxLines = Number(base?.global?.maxLines || DEFAULT_TRUNCATION_POLICY.global.maxLines);
  const priority = Number(base?.global?.priority || DEFAULT_TRUNCATION_POLICY.global.priority);
  return {
    global: { strategy, maxLines: Number.isFinite(maxLines) ? maxLines : 3, priority: Number.isFinite(priority) ? priority : 2 },
    priorityOrder: Array.isArray(base.priorityOrder) && base.priorityOrder.length ? base.priorityOrder.filter((f) => TRUNCATION_FIELDS.includes(f)) : [...DEFAULT_TRUNCATION_POLICY.priorityOrder],
    surfaceBehavior: {
      ...DEFAULT_TRUNCATION_POLICY.surfaceBehavior,
      ...(base.surfaceBehavior || {}),
    },
    strictModeSafe: Boolean(base.strictModeSafe),
    fields: base.fields && typeof base.fields === 'object' ? base.fields : {},
  };
};
