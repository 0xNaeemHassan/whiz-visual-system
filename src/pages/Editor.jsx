...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const hasFreshnessWarnings = useMemo(() => freshnessReport.warnings.length > 0, [freshnessReport.warnings.length]);
  const signoffComplete = useMemo(() => hasCompletedSignoff(signoffRecord, reviewState), [signoffRecord, reviewState]);
  ...
  const exportSummarySidecars=(baseName,summary,imageFileName)=>{const payload={...summary,imageFileName,trustLevel};downloadFile(`${baseName}.summary.json`,new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));downloadFile(`${baseName}.summary.txt`,new Blob([buildSummaryText(payload)],{type:'text/plain'}));downloadFile(`${baseName}.manifest.signoff.json`,new Blob([JSON.stringify({reviewState,signoffRecord,trustLevel,savedAt:new Date().toISOString()},null,2)],{type:'application/json'}));};
