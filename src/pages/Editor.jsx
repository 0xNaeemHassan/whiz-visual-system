...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const exportJSON=async()=>{if(!validateExportTypography())return;if(!validateCriticalExportFields('json'))return;if(!ensureActionAllowed('export'))return;const normalized=await runNormalizationPreflight();if(!normalized)return;const checksum=await buildAuditChecksum();const exportPayload={...buildSave(),content:{...content,...normalized},auditTrail,auditChecksum:checksum};const d=JSON.stringify(exportPayload,null,2);const b=new Blob([d],{type:'application/json'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download=`${normalized.slug||'whiz_export'}.json`;a.click();URL.revokeObjectURL(u);appendAuditEvent({action:'export',section:'editor',fieldPath:'export.json',oldValueHash:'n/a',newValueHash:hashValue(exportPayload),context:{format:'json',checksum}});showToast('JSON exported');};
  const exportManifest=async()=>{if(!validateExportTypography())return;if(!validateCriticalExportFields('manifest'))return;if(!ensureActionAllowed('publish'))return;if(hasBlockingSpineContrastIssue){showToast('Publish blocked: rotated spine contrast is below threshold.','error');return;}if(editorValidation.errors.length){showToast(`Export blocked: ${editorValidation.errors.join(' | ')}`,'error');return;}if(editorValidation.warnings.length&&!window.confirm(`Editor validation warnings:\n- ${editorValidation.warnings.join('\n- ')}\n\nExport anyway?`))return;const normalized=await runNormalizationPreflight();if(!normalized)return;const checksum=await buildAuditChecksum();const payload={issueNum:content.issueNum,topic:normalized.topicTag,slug:normalized.slug,title:content.title,date:content.date,frameId,themeId:theme.id,strictMode,brandScore,complianceIssues,preflightResult,activityLog,sources:(content.sourceLinks||'').split(',').map(s=>s.trim()).filter(Boolean),targetMetric:content.targetMetric||'',metricConfidence:content.metricConfidence||'',metricProvenance:Array.isArray(content.metricProvenance)?content.metricProvenance:(content.metricProvenance?[content.metricProvenance]:[]),rowProvenance:{stats:normalizeStatsWithProvenance(content.stats).map((s,idx)=>({index:idx,label:s.label,provenance:s.provenance})),tableRows:normalizeTableRowsWithProvenance(content.tableRows).map((row,idx)=>({index:idx,cells:Object.fromEntries(Object.entries(row).filter(([k])=>k!=='provenance')),provenance:row.provenance}))},auditTrail,auditChecksum:checksum,exportedAt:new Date().toISOString()};const d=JSON.stringify(payload,null,2);const b=new Blob([d],{type:'application/json'});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download=`whiz_issue${content.issueNum||'000'}_manifest.json`;a.click();URL.revokeObjectURL(u);appendAuditEvent({action:'publish',section:'editor',fieldPath:'export.manifest',oldValueHash:'n/a',newValueHash:hashValue(payload),context:{format:'manifest',checksum}});showToast('Manifest exported');};
  ...
  const updateContent=(k,v,forceImmediate=false)=>{const oldHash=hashValue(content?.[k]);const did=mutations.content(k,c=>{const next={...c,[k]:v};if(k==='date'){const normalized=normalizeDateInput(v);if(normalized.valid)return{...next,date:normalized.displayDate};}if(k==='timelineEvents'){return{...next,timelineEvents:normalizeTimelineEvents(v)};}if(k==='topicTag'||k==='slug'){return normalizeContentTaxonomy(next).content;}if(['bigNumber','bigValue','targetMetric','stats','tableRows'].includes(k)){return normalizeNumericFields(next).content;}return next;},forceImmediate);appendAuditEvent({action:'edit',section:'content',fieldPath:`content.${k}`,oldValueHash:oldHash,newValueHash:hashValue(v),context:{mutation:'content',forceImmediate:Boolean(forceImmediate)}});return did;};
  ...
  const updateStyle=(updater)=>{const prevHash=hashValue(overrides);const did=mutations.style(updater);appendAuditEvent({action:'override',section:'overrides',fieldPath:'overrides',oldValueHash:prevHash,newValueHash:hashValue(overrides),context:{mutation:'style'}});return did;};
