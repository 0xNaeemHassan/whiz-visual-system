...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      perfMeasure('editor.initial-render', start, {
        tier: selectedFrame?.tier,
        metric: 'initialRenderMs',
        extra: { frameId: selectedFrame?.id, layout: selectedFrame?.layout },
      });
    });
    return () => cancelAnimationFrame(raf);
  }, [selectedFrame?.id, selectedFrame?.layout, content, overrides, aspectRatio]);
  ...
  const exportPNG=async()=>{if(!frameRef.current||exporting)return;const exportStart=perfNow();if(strictMode&&complianceIssues.length){showToast(`Strict mode blocked export (${complianceIssues.length} issues)`,'error');return;}if(!strictMode&&complianceIssues.length&&!window.confirm(`Whiz compliance warnings:\\n- ${complianceIssues.join('\\n- ')}\\n\\nExport anyway?`))return;setExporting(true);showToast('Generating PNG...','info');try{const h2c=(await import('html2canvas')).default;const c=frameRef.current.cloneNode(true);c.style.cssText=`position:absolute;left:-9999px;top:0;transform:none;width:${aspectRatio.w}px;height:${aspectRatio.h}px`;document.body.appendChild(c);let cv;try{cv=await h2c(c,{scale:2,useCORS:true,allowTaint:true,width:aspectRatio.w,height:aspectRatio.h,backgroundColor:overrides.frameBg||theme.base,logging:false});}finally{if(document.body.contains(c))document.body.removeChild(c);}try{cv.toBlob(bl=>{bl&&navigator.clipboard?.write&&navigator.clipboard.write([new ClipboardItem({'image/png':bl})]).catch(()=>{});});}catch(e){}const u=cv.toDataURL('image/png');const a=document.createElement('a');a.href=u;a.download=`whiz_export.png`;a.click();showToast(`PNG exported at 2x`);perfMeasure('export.png',exportStart,{tier:selectedFrame?.tier,metric:'exportMs',extra:{format:'png',frameId:selectedFrame?.id}});}catch(e){console.error(e);showToast(`Export failed: ${e.message||'unknown error'}`,'error');}setExporting(false);}
  const exportWebP=async()=>{
    ...
    try{
      ...
      await new Promise((res,rej)=>cv.toBlob(b=>{
        ...
        showToast('WebP exported');perfMeasure('export.webp',exportStart,{tier:selectedFrame?.tier,metric:'exportMs',extra:{format:'webp',frameId:selectedFrame?.id}});res();
      },'image/webp',0.92));
    }catch(e){showToast(`WebP failed: ${e.message||'error'}`,'error');}
    setExporting(false);
  };
  const updateContent=(k,v)=>{const start=perfNow();setContent(c=>({...c,[k]:v}));perfMeasure('editor.interaction.update-content',start,{tier:selectedFrame?.tier,metric:'interactionLatencyMs',extra:{field:k}});};
