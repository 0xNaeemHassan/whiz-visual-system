...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const [collisionResolutions, setCollisionResolutions] = useState({});
  ...
  const runTableImportValidation = () => {
    ...
    track(TELEMETRY_EVENTS.DUPLICATE_RESOLUTION,{source:'import_validate',duplicateGroups:report.collisionReport?.duplicateGroups?.length||0,fuzzyPairs:report.collisionReport?.fuzzyPairs?.length||0,unresolvedHighConfidence:(report.collisionReport?.fuzzyPairs||[]).filter((pair)=>pair.confidence>=0.9).length});
