...
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><div className="form-group" style={{marginBottom:0}}><label className="form-label">@X</label><input value={content.socialX||''} onChange={e=>updateContent('socialX',e.target.value)}/></div><div className="form-group" style={{marginBottom:0}}><label className="form-label">@Sub</label><input value={content.socialSub||''} onChange={e=>updateContent('socialSub',e.target.value)}/></div></div></div>
          <div className="editor-section"><div className="editor-section-title">Editorial</div><div className="form-group"><label className="form-label">Title</label><textarea value={content.title} onChange={e=>updateContent('title',e.target.value)} rows={2}/><div className={`char-count ${content.title.length>60?'warn':''} ${content.title.length>80?'over':''}`}>{content.title.length}/60</div></div><div className="form-group"><label className="form-label">Deck</label><textarea value={content.deck} onChange={e=>updateContent('deck',e.target.value)} rows={2}/><div className={`char-count ${content.deck.length>120?'warn':''}`}>{content.deck.length}/120</div></div><div className="form-group"><label className="form-label">Body</label><textarea value={content.body} onChange={e=>updateContent('body',e.target.value)} rows={4}/><div className={`char-count ${content.body.length>400?'warn':''}`}>{content.body.length}/400</div></div>{frameId===48&&<div className="form-group"><label className="form-label">Species</label><textarea value={content.species||''} onChange={e=>updateContent('species',e.target.value)} rows={1} placeholder="Name one archetype (e.g., Yield Farmer)"/><div className="hint-box">Prompt: Use a concise singular label and avoid hype nouns.</div></div>}{frameId===48&&<div className="form-group"><label className="form-label">Habitat</label><textarea value={content.habitat||''} onChange={e=>updateContent('habitat',e.target.value)} rows={2} placeholder="Where this actor is most commonly found on-chain"/><div className="hint-box">Prompt: Describe chains/protocol zones in objective terms.</div></div>}{frameId===48&&<div className="form-group"><label className="form-label">Diet</label><textarea value={content.diet||''} onChange={e=>updateContent('diet',e.target.value)} rows={2} placeholder="What incentives or opportunities this actor consumes"/><div className="hint-box">Prompt: Focus on mechanisms (fees, points, emissions), not opinions.</div></div>}{frameId===48&&<div className="form-group"><label className="form-label">Spotting Tips</label><textarea value={content.spottingTips||''} onChange={e=>updateContent('spottingTips',e.target.value)} rows={2} placeholder="Observable signals that reveal this archetype"/><div className="hint-box">Prompt: Start with an action verb and cite visible behavior.</div></div>}{frameId===48&&<div className="form-group"><label className="form-label">Behavior Note</label><textarea value={content.behaviorNote||''} onChange={e=>updateContent('behaviorNote',e.target.value)} rows={2} placeholder="How this actor behaves over time"/><div className="hint-box">Prompt: Keep tone analytical, one sentence, present tense.</div></div>}</div>
