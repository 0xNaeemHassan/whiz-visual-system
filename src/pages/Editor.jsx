...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const[showDeleteConfirm,setShowDeleteConfirm]=useState(null);const[saveSearch,setSaveSearch]=useState('');const[openProvenancePanels,setOpenProvenancePanels]=useState({});
...
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><div className="form-group" style={{marginBottom:0}}><label className="form-label">@X</label><input value={content.socialX||''} onChange={e=>updateContent('socialX',e.target.value)}/></div><div className="form-group" style={{marginBottom:0}}><label className="form-label">@Sub</label><input value={content.socialSub||''} onChange={e=>updateContent('socialSub',e.target.value)}/></div></div></div>
          ...
          <div className="editor-section"><div className="editor-section-title">Stats</div><div role="list">{content.stats.map((s,i)=>{const panelKey=`stat-${i}`;const prov=normalizeProvenance(s?.provenance);const provReady=isPublishReadyProvenance(prov);return(<DragItem key={`s-${i}`} index={i} id={`s-${i}`} onMove={(f,t)=>{const a=[...content.stats];const[it]=a.splice(f,1);a.splice(t,0,it);updateContent('stats',a);}}><div style={{display:'grid',gridTemplateColumns:'auto 1fr 1fr auto',gap:6,marginBottom:4,alignItems:'center'}}><span style={{color:'var(--dim)',fontSize:10,cursor:'grab',padding:'0 4px'}}>⋮⋮</span><input value={s.label} placeholder="LABEL" style={{fontSize:11}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],label:e.target.value};updateContent('stats',a);}}/><input value={s.value} placeholder="VALUE" style={{fontSize:11}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],value:e.target.value};updateContent('stats',a);}}/><button className="btn btn-danger btn-sm" style={{padding:'0 6px',height:28}} onClick={()=>updateContent('stats',content.stats.filter((_,r)=>r!==i))}>✕</button></div><details open={Boolean(openProvenancePanels[panelKey])} onToggle={e=>setOpenProvenancePanels(prev=>({...prev,[panelKey]:e.target.open}))} style={{marginBottom:6,border:'1px solid var(--line)',borderRadius:6,padding:'6px 8px'}}><summary style={{cursor:'pointer',fontSize:11,color:provReady?'var(--muted)':'#FFB86B'}}>Provenance {provReady?'✓':'• required for publish'}</summary><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginTop:6}}><input value={prov.source} placeholder="Source" style={{fontSize:10}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],provenance:{...prov,source:e.target.value}};updateContent('stats',a);}}/><input type="date" value={prov.date} style={{fontSize:10}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],provenance:{...prov,date:e.target.value}};updateContent('stats',a);}}/><select value={prov.confidence} style={{fontSize:10}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],provenance:{...prov,confidence:e.target.value}};updateContent('stats',a);}}>{PROVENANCE_CONFIDENCE_OPTIONS.map(level=><option key={level} value={level}>{level.toUpperCase()}</option>)}</select><input value={prov.links} placeholder="Links (comma-separated)" style={{fontSize:10}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],provenance:{...prov,links:e.target.value}};updateContent('stats',a);}}/><textarea value={prov.notes} rows={2} placeholder="Notes" style={{gridColumn:'1 / -1',fontSize:10}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],provenance:{...prov,notes:e.target.value}};updateContent('stats',a);}}/></div></details></DragItem>);})}</div><button className="btn btn-secondary btn-sm w-full" style={{marginTop:4}} onClick={()=>updateContent('stats',[...content.stats,{label:'',value:'',provenance:createDefaultProvenance()}])}>+ Stat</button></div>
...
<div className="editor-section"><div className="editor-section-title">Table</div><div className="form-group"><label className="form-label">Headers (comma-sep)</label><input value={(content.tableHeaders||[]).join(', ')} onChange={e=>updateContent('tableHeaders',e.target.value.split(',').map(s=>s.trim()).filter(Boolean))}/></div>{(content.tableRows||[]).map((row,i)=>{const panelKey=`table-${i}`;const prov=normalizeProvenance(row?.provenance);const provReady=isPublishReadyProvenance(prov);const rowKeys=Object.keys(row||{}).filter(k=>k!=='provenance');return(<DragItem key={`tr-${i}`} index={i} id={`tr-${i}`} onMove={(f,t)=>{const a=[...content.tableRows];const[it]=a.splice(f,1);a.splice(t,0,it);updateContent('tableRows',a);}}><div style={{display:'grid',gridTemplateColumns:`auto repeat(${rowKeys.length},1fr) auto`,gap:3,marginBottom:3,alignItems:'center'}}><span style={{color:'var(--dim)',fontSize:10,cursor:'grab'}}>⋮⋮</span>{rowKeys.map(k=>(<input key={k} value={row[k]} style={{fontSize:10,padding:'5px 6px'}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],[k]:e.target.value};updateContent('tableRows',a);}}/>))}<button className="btn btn-danger btn-sm" style={{padding:'0 7px',height:30}} onClick={()=>updateContent('tableRows',content.tableRows.filter((_,r)=>r!==i))}>✕</button></div><details open={Boolean(openProvenancePanels[panelKey])} onToggle={e=>setOpenProvenancePanels(prev=>({...prev,[panelKey]:e.target.open}))} style={{marginBottom:6,border:'1px solid var(--line)',borderRadius:6,padding:'6px 8px'}}><summary style={{cursor:'pointer',fontSize:11,color:provReady?'var(--muted)':'#FFB86B'}}>Provenance {provReady?'✓':'• required for publish'}</summary><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6,marginTop:6}}><input value={prov.source} placeholder="Source" style={{fontSize:10}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],provenance:{...prov,source:e.target.value}};updateContent('tableRows',a);}}/><input type="date" value={prov.date} style={{fontSize:10}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],provenance:{...prov,date:e.target.value}};updateContent('tableRows',a);}}/><select value={prov.confidence} style={{fontSize:10}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],provenance:{...prov,confidence:e.target.value}};updateContent('tableRows',a);}}>{PROVENANCE_CONFIDENCE_OPTIONS.map(level=><option key={level} value={level}>{level.toUpperCase()}</option>)}</select><input value={prov.links} placeholder="Links (comma-separated)" style={{fontSize:10}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],provenance:{...prov,links:e.target.value}};updateContent('tableRows',a);}}/><textarea value={prov.notes} rows={2} placeholder="Notes" style={{gridColumn:'1 / -1',fontSize:10}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],provenance:{...prov,notes:e.target.value}};updateContent('tableRows',a);}}/></div></details></DragItem>);})}<button className="btn btn-secondary btn-sm w-full" style={{marginTop:6}} onClick={()=>{const c=(content.tableHeaders||[]).length||5;const r={};for(let j=1;j<=c;j++)r[`col${j}`]='';r.provenance=createDefaultProvenance();updateContent('tableRows',[...(content.tableRows||[]),r]);}}>+ Row</button></div>
          <div className="editor-section">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
