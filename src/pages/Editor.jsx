...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const [editorMode, setEditorMode] = useLocalStorage('whiz-editor-mode', 'novice');
  const [noviceStep, setNoviceStep] = useLocalStorage('whiz-novice-step', 'outline');
...
Apply these auto-corrections?`);
  ...
  return(
    <div className="editor-wrap">
      ...
      {!isExpertMode && <div className="editor-section" style={{margin:'8px 0',padding:'8px 12px'}}>
        <div style={{fontFamily:'var(--font-m)',fontSize:10,color:'var(--dim)',marginBottom:6}}>Guided Steps</div>
        <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          {noviceSteps.map((step, index) => (
            <button key={step.id} className={`btn btn-sm ${noviceStep===step.id?'btn-primary':'btn-ghost'}`} onClick={()=>setNoviceStep(step.id)} title={step.hint}>
              {index + 1}. {step.label}
            </button>
          ))}
        </div>
        <div style={{marginTop:8,fontSize:11,color:'var(--muted)'}}>
          <strong>Hint:</strong> {noviceSteps.find((step) => step.id === noviceStep)?.hint} · <strong>Why this matters:</strong> {noviceSteps.find((step) => step.id === noviceStep)?.why}
        </div>
      </div>}
