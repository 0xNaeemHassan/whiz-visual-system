...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const buildSave=()=>buildFrameSave({frameId,theme,content,overrides,aspectRatio,bgGradient,patternOverlay,tags:saveTagsInput.split(',').map(t=>t.trim()).filter(Boolean),folder:saveFolder.trim(),status:saveStatus.trim()||undefined});
  const doSave=()=>{const n=saveName.trim()||`${content.topicTag} \u2014 ${new Date().toLocaleDateString()}`;setSaves(p=>[...p,{id:`s_${Date.now()}`,title:n,...buildSave()}]);setShowSaveModal(false);setSaveName('');setSaveTagsInput('');setSaveFolder('');setSaveStatus('');showToast(`Saved "${n}"`);};
  ...
  const filteredSaves = saves.slice().reverse().filter((s) => {
    const q = saveSearch.trim().toLowerCase();
    const nameOk = !q || (s.title || '').toLowerCase().includes(q);
    const tagQ = loadTagFilter.trim().toLowerCase();
    const tags = Array.isArray(s.tags) ? s.tags : [];
    const tagOk = !tagQ || tags.some((t) => (t || '').toLowerCase().includes(tagQ));
    const folderOk = !loadFolderFilter.trim() || (s.folder || '').toLowerCase().includes(loadFolderFilter.trim().toLowerCase());
    const frameOk = !loadFrameFilter || String(s.frameId || '') === loadFrameFilter;
    const ts = Number(s.savedAt || 0);
    const fromOk = !loadDateFrom || ts >= new Date(`${loadDateFrom}T00:00:00`).getTime();
    const toOk = !loadDateTo || ts <= new Date(`${loadDateTo}T23:59:59`).getTime();
    return nameOk && tagOk && folderOk && frameOk && fromOk && toOk;
  });
