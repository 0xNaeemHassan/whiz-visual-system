...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const loadSave=s=>{setFrameId(s.frameId);setTheme(s.theme);resetContent(s.content);if(s.overrides){const validation=validateEditorState({frameId:s.frameId,theme:s.theme,content:s.content||{},overrides:s.overrides},{strictMode:Boolean(strictMode),sanitizeStrictStyle:true});const next=validation.sanitizedOverrides||s.overrides;setOverrides(next);if(strictMode&&validation.codes.includes('STRICT_STYLE_OVERRIDE_BLOCKED')){showToast('Loaded with strict-style sanitization.','warning');}}s.aspectRatio&&setAspectRatio(s.aspectRatio);s.bgGradient&&updateMedia(prev=>({...prev,bgGradient:s.bgGradient}));s.patternOverlay&&updateMedia(prev=>({...prev,patternOverlay:s.patternOverlay}));setSelectedSaveForLoad(null);setShowLoadModal(false);showToast(`Loaded`);};
  const currentEditorSave=useMemo(()=>buildSave(),[frameId,theme,content,overrides,aspectRatio,bgGradient,patternOverlay]);
  const selectedSaveDiff=useMemo(()=>buildSaveDiff(currentEditorSave,selectedSaveForLoad),[currentEditorSave,selectedSaveForLoad]);
  const previewSelectedSave=()=>{if(!selectedSaveForLoad){showToast('Select a save to preview.','warning');return;}const d=selectedSaveDiff;const warned=d?.destructive?.length?`\n\n⚠ ${d.destructive.join(' ')}`:'';window.alert(`Preview: ${selectedSaveForLoad.title}\n\nContent changes: ${d?.sections?.content?.count||0}\nTheme: ${d?.sections?.theme?.changed?'yes':'no'}\nOverrides: ${d?.sections?.overrides?.changed?'yes':'no'}\nLayout: ${d?.sections?.layout?.changed?'yes':'no'}${warned}`);};
  const confirmAndLoad=()=>{if(!selectedSaveForLoad){showToast('Select a save to load.','warning');return;}const d=selectedSaveDiff;const destructiveText=d?.destructive?.length?`\n\nDestructive changes:\n- ${d.destructive.join('\n- ')}`:'';if(!window.confirm(`Overwrite current editor state with "${selectedSaveForLoad.title}"?${destructiveText}`))return;loadSave(selectedSaveForLoad);};
...
<div className="editor-section"><div className="editor-section-title">Table</div><div className="form-group"><label className="form-label">Headers (comma-sep)</label><input value={(content.tableHeaders||[]).join(', ')} onChange={e=>updateContent('tableHeaders',e.target.value.split(',').map(s=>s.trim()).filter(Boolean))}/></div>{(content.tableRows||[]).map((row,i)=>(<DragItem key={`tr-${i}`} index={i} id={`tr-${i}`} onMove={(f,t)=>{const a=[...content.tableRows];const[it]=a.splice(f,1);a.splice(t,0,it);updateContent('tableRows',a);}}><div style={{display:'grid',gridTemplateColumns:`auto repeat(${Object.keys(row).length},1fr) auto`,gap:3,marginBottom:3,alignItems:'center'}}><span style={{color:'var(--dim)',fontSize:10,cursor:'grab'}}>⋮⋮</span>{Object.keys(row).map(k=>(<input key={k} value={row[k]} style={{fontSize:10,padding:'5px 6px'}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],[k]:e.target.value};updateContent('tableRows',a);}}/>))}<button className="btn btn-danger btn-sm" style={{padding:'0 7px',height:30}} onClick={()=>updateContent('tableRows',content.tableRows.filter((_,r)=>r!==i))}>✕</button></div></DragItem>))}<button className="btn btn-secondary btn-sm w-full" style={{marginTop:6}} onClick={()=>{const c=(content.tableHeaders||[]).length||5;const r={};for(let j=1;j<=c;j++)r[`col${j}`]='';updateContent('tableRows',[...(content.tableRows||[]),r]);}}>+ Row</button></div>
      ...
      {showLoadModal&&<div className="modal-overlay open" onClick={e=>e.target===e.currentTarget&&setShowLoadModal(false)}><div className="modal"><div className="modal-header"><span className="modal-title">Load</span><button className="modal-close" onClick={()=>setShowLoadModal(false)}>✕</button></div>{saves.length===0?<div className="empty-state">No saves.</div>:<><div className="saves-list" style={{maxHeight:260,overflowY:'auto'}}>{saves.slice().reverse().map(s=>(<div key={s.id} className={`save-item ${selectedSaveForLoad?.id===s.id?'active':''}`} onClick={()=>setSelectedSaveForLoad(s)}><div style={{width:32,height:32,borderRadius:6,background:s.theme?.base||'var(--bg-3)',border:`2px solid ${s.theme?.accent||'var(--border)'}`,flexShrink:0}}/><div className="save-item-info"><div className="save-item-name">{s.title}</div><div className="save-item-meta">#{s.frameId} · {new Date(s.savedAt).toLocaleDateString()}</div></div><button className="btn btn-danger btn-sm" onClick={e=>{e.stopPropagation();setShowDeleteConfirm(s.id);}}>✕</button></div>))}</div><div className="editor-section" style={{marginTop:10}}><div className="editor-section-title">Change Summary</div>{!selectedSaveForLoad?<div style={{fontSize:11,color:'var(--dim)'}}>Select a save to preview differences.</div>:<div style={{display:'flex',flexDirection:'column',gap:6,fontSize:11}}><div>Content: <strong>{selectedSaveDiff?.sections?.content?.count||0} changes</strong></div><div>Theme: <strong>{selectedSaveDiff?.sections?.theme?.changed?'Changed':'No change'}</strong></div><div>Overrides: <strong>{selectedSaveDiff?.sections?.overrides?.changed?'Changed':'No change'}</strong></div><div>Layout: <strong>{selectedSaveDiff?.sections?.layout?.changed?'Changed':'No change'}</strong></div>{(selectedSaveDiff?.destructive?.length||0)>0&&<div style={{padding:'8px',borderRadius:6,border:'1px solid rgba(235,87,87,0.45)',background:'rgba(235,87,87,0.08)'}}><div style={{fontFamily:'var(--font-m)',fontSize:9,textTransform:'uppercase',marginBottom:4}}>Destructive differences</div><ul style={{margin:0,paddingLeft:16}}>{selectedSaveDiff.destructive.map((msg,idx)=><li key={idx}>{msg}</li>)}</ul></div>}</div>}</div><div className="modal-footer"><button className="btn btn-secondary" onClick={previewSelectedSave} disabled={!selectedSaveForLoad}>Preview</button><button className="btn btn-primary" onClick={confirmAndLoad} disabled={!selectedSaveForLoad}>Overwrite & Load</button></div></>}</div></div>}{showDeleteConfirm&&<div className="confirm-overlay" onClick={()=>setShowDeleteConfirm(null)}><div className="confirm-box" onClick={e=>e.stopPropagation()}><div className="confirm-title">Delete?</div><div className="confirm-actions"><button className="btn btn-secondary btn-sm" onClick={()=>setShowDeleteConfirm(null)}>Cancel</button><button className="btn btn-danger btn-sm" onClick={confirmDel}>Delete</button></div></div></div>}
