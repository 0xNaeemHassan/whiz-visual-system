...
const DEFAULT_DUPLICATE_WIZARD = {
  enabled: false,
  title: '',
  topicTag: '',
  publishDate: '',
...
const buildPlannerIssueDraftFromEditor = ({ frameId, theme, content, wizard = DEFAULT_DUPLICATE_WIZARD }) => {
  const sourceLinks = (content.sourceLinks || '').trim();
  return {
    issueNum: String(content.issueNum || '').replace(/\D/g, '').slice(-3).padStart(3, '0') || '000',
    topic: (wizard.title || content.title || content.topicTag || 'Untitled frame').trim(),
    frameId: String(frameId || ''),
    themeId: theme?.id || '',
    status: 'draft',
    priority: 'medium',
    publishDate: wizard.publishDate || '',
    notes: content.body || '',
    caption: content.deck || '',
    sourceLinks,
    confidence: wizard.confidence || 'medium',
    series: (wizard.topicTag || content.topicTag || '').trim(),
    assistantBrief: content.thesis || '',
    targetMetric: content.bigLabel || '',
    metricValue: content.bigNumber || '',
    metricUnit: '',
    metricSource: sourceLinks ? 'editor' : '',
    metricConfidence: wizard.confidence || 'medium',
    metricProvenance: wizard.provenanceRequired ? ['sourceLinks'] : [],
    createdAt: Date.now(),
  };
};
...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const[showIssueDraftModal,setShowIssueDraftModal]=useState(false);const[showDuplicateWizard,setShowDuplicateWizard]=useState(false);
  const[duplicateWizard,setDuplicateWizard]=useState(DEFAULT_DUPLICATE_WIZARD);
...
<div className="editor-section"><div className="editor-section-title">Table</div><div className="form-group"><label className="form-label">Headers (comma-sep)</label><input value={(content.tableHeaders||[]).join(', ')} onChange={e=>updateContent('tableHeaders',e.target.value.split(',').map(s=>s.trim()).filter(Boolean))}/></div>{(content.tableRows||[]).map((row,i)=>(<DragItem key={`tr-${i}`} index={i} id={`tr-${i}`} onMove={(f,t)=>{const a=[...content.tableRows];const[it]=a.splice(f,1);a.splice(t,0,it);updateContent('tableRows',a);}}><div style={{display:'grid',gridTemplateColumns:`auto repeat(${Object.keys(row).length},1fr) auto`,gap:3,marginBottom:3,alignItems:'center'}}><span style={{color:'var(--dim)',fontSize:10,cursor:'grab'}}>⋮⋮</span>{Object.keys(row).map(k=>(<input key={k} value={row[k]} style={{fontSize:10,padding:'5px 6px'}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],[k]:e.target.value};updateContent('tableRows',a);}}/>))}<button className="btn btn-danger btn-sm" style={{padding:'0 7px',height:30}} onClick={()=>updateContent('tableRows',content.tableRows.filter((_,r)=>r!==i))}>✕</button></div></DragItem>))}<button className="btn btn-secondary btn-sm w-full" style={{marginTop:6}} onClick={()=>{const c=(content.tableHeaders||[]).length||5;const r={};for(let j=1;j<=c;j++)r[`col${j}`]='';updateContent('tableRows',[...(content.tableRows||[]),r]);}}>+ Row</button></div>
          ...
          <div className="editor-section"><div className="editor-section-title">Frame Warnings</div>{activeFramePitfalls.length===0?<div style={{fontSize:11,color:'var(--dim)'}}>No known pitfalls for this frame.</div>:<div style={{display:'flex',flexDirection:'column',gap:6}}>{activeFramePitfalls.map(p=>(<div key={p.id} style={{fontSize:11,padding:'7px 8px',borderRadius:6,border:`1px solid ${p.severity==='high'?'rgba(235,87,87,0.45)':'rgba(229,178,58,0.45)'}`,background:p.severity==='high'?'rgba(235,87,87,0.08)':'rgba(229,178,58,0.1)'}}><div style={{fontFamily:'var(--font-m)',fontSize:9,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:4}}>{p.severity==='high'?'Blocking':'Non-blocking'}</div><div>{p.warning}</div><div style={{opacity:0.75,marginTop:4}}>Hint: {p.triggerHint}</div></div>))}</div>}</div><div className="editor-section"><div className="editor-section-title">Export</div><div style={{fontFamily:'var(--font-m)',fontSize:9,color:'var(--dim)',marginBottom:8,lineHeight:1.7}}>⌘S Save · ⌘Z Undo · ⌘⇧Z Redo</div><div style={{display:'flex',flexDirection:'column',gap:8}}><div style={{display:'flex',gap:6}}><button className="btn btn-primary btn-sm" style={{flex:1}} onClick={exportPNG} disabled={exporting}>{exporting?'…':`PNG`}</button><button className="btn btn-secondary btn-sm editor-action-btn" onClick={exportWebP} disabled={exporting}>WebP</button></div><div style={{display:'flex',gap:6}}><button className="btn btn-ghost btn-sm" style={{flex:1}} onClick={exportHTML}>HTML</button><button className="btn btn-ghost btn-sm" style={{flex:1}} onClick={exportJSON}>JSON</button></div><label className="import-label">↑ Import <input type="file" accept=".json" onChange={importJSON}/></label><button className="btn btn-secondary w-full" onClick={()=>setShowSaveModal(true)}>Save</button><button className="btn btn-ghost w-full" onClick={()=>{const n=`${content.title||'Frame'} (copy)`;setSaves(p=>[...p,{id:`s_${Date.now()}`,title:n,...buildSave()}]);setDuplicateWizard({ ...DEFAULT_DUPLICATE_WIZARD, title: `${content.title||'Frame'} (copy)`, topicTag: content.topicTag||'' });setShowIssueDraftModal(true);track(TELEMETRY_EVENTS.DUPLICATE_AUTOPILOT_METRIC,{stage:'duplicated',completed:true,withWizard:false});showToast('Duplicated');}}>Duplicate</button>{saves.length>0&&<button className="btn btn-ghost w-full" onClick={()=>{setSaveSearch('');setShowLoadModal(true);}}>Load ({saves.length})</button>}</div></div>
        </>)}
      </div>
      {/* MODALS */}
      ...
      {showIssueDraftModal&&<div className="modal-overlay open" onClick={e=>e.target===e.currentTarget&&setShowIssueDraftModal(false)}><div className="modal"><div className="modal-header"><span className="modal-title">Create issue draft</span><button className="modal-close" onClick={()=>setShowIssueDraftModal(false)}>✕</button></div><p style={{fontSize:12,color:'var(--muted)',marginBottom:12}}>Send this duplicated frame to Planner with pre-filled metadata.</p><label style={{display:'flex',alignItems:'center',gap:8,fontSize:12,marginBottom:12}}><input type="checkbox" checked={showDuplicateWizard} onChange={e=>{setShowDuplicateWizard(e.target.checked);setDuplicateWizard(prev=>({...prev,enabled:e.target.checked}));}}/>Open post-duplicate wizard</label>{showDuplicateWizard&&<div style={{display:'grid',gap:8}}><input value={duplicateWizard.title} onChange={e=>setDuplicateWizard(prev=>({...prev,title:e.target.value}))} placeholder="Retitle"/><input value={duplicateWizard.topicTag} onChange={e=>setDuplicateWizard(prev=>({...prev,topicTag:e.target.value}))} placeholder="Retag / series"/><input type="date" value={duplicateWizard.publishDate} onChange={e=>setDuplicateWizard(prev=>({...prev,publishDate:e.target.value}))}/><select value={duplicateWizard.confidence} onChange={e=>setDuplicateWizard(prev=>({...prev,confidence:e.target.value}))}><option value="low">Low confidence</option><option value="medium">Medium confidence</option><option value="high">High confidence</option></select><label style={{display:'flex',alignItems:'center',gap:8,fontSize:12}}><input type="checkbox" checked={duplicateWizard.provenanceRequired} onChange={e=>setDuplicateWizard(prev=>({...prev,provenanceRequired:e.target.checked}))}/>Require provenance links</label></div>}<div className="modal-footer"><button className="btn btn-secondary" onClick={()=>setShowIssueDraftModal(false)}>Skip</button><button className="btn btn-primary" onClick={()=>{const payload=buildPlannerIssueDraftFromEditor({frameId,theme,content,wizard:duplicateWizard});localStorage.setItem('whiz-planner-issue-draft',JSON.stringify(payload));setShowIssueDraftModal(false);setShowDuplicateWizard(false);navigateTo?.('planner');track(TELEMETRY_EVENTS.DUPLICATE_AUTOPILOT_METRIC,{stage:'issue_draft_created',completed:true,withWizard:showDuplicateWizard,confidence:payload.confidence,provenanceRequired:duplicateWizard.provenanceRequired});showToast('Planner draft prepared');}}>Create issue draft</button></div></div></div>}
