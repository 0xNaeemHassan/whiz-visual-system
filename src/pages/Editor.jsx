...
import { isStrictStylePathAllowed, getStrictStyleBlockReason, sanitizeStrictStyleOverrides } from '../domain/strictStylePolicy';
...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const importJSON=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=ev=>{try{const d=JSON.parse(ev.target.result);const validation=validateEditorState({content:d.content||{},overrides:d.overrides||{},uploadedImages:d.uploadedImages||null},{strictMode:Boolean(strictMode),sanitizeStrictStyle:true});if(!validation.valid&&!validation.codes.every(code=>code==='STRICT_STYLE_OVERRIDE_BLOCKED')){showToast(`Import rejected (${validation.codes.join(', ')})`,'error');return;}d.frameId&&setFrameId(d.frameId);d.theme&&(setTheme(d.theme),setActiveTheme(d.theme));d.content&&resetContent(d.content);if(d.overrides){setOverrides(validation.sanitizedOverrides||d.overrides);if(strictMode&&validation.codes.includes('STRICT_STYLE_OVERRIDE_BLOCKED')){showToast('Imported with strict-style sanitization.','warning');}}d.aspectRatio&&setAspectRatio(d.aspectRatio);d.bgGradient&&setBgGradient(d.bgGradient);d.patternOverlay&&setPatternOverlay(d.patternOverlay);showToast('Imported');}catch(err){showToast('Invalid JSON','error');}};r.readAsText(f);e.target.value='';};
