...
import { applyOverflowPolicy } from '../components/whizframe/layouts/OverflowPolicy';
    ...
    <div className="editor-wrap">
      ...
      <div className={`editor-right ${mobileTab==='content'?'mob-active':''}`}>
        ...
        {rightTab==='design'?(<><div className={`edit-toggle-row ${editMode?'on':''}`} onClick={()=>{setEditMode(m=>!m);editMode&&setSelectedEl(null);}}><div className={`toggle-pill ${editMode?'on':''}`}><div className="toggle-dot"/></div><span className="toggle-label">{editMode?'Edit ON':'Edit OFF'}</span><span className="toggle-hint">{editMode?'Click elements':'Toggle to edit'}</span></div><DesignPanel selectedEl={selectedEl} setSelectedEl={setSelectedEl} overrides={overrides} setOverrides={setOverrides} theme={theme} bgGradient={bgGradient} setBgGradient={setBgGradient} showToast={showToast} resetOverrides={resetOverrides} setPatternOverlay={setPatternOverlay} strictMode={strictMode}/>
          ...
          <div className="editor-section"><div className="editor-section-title">Truncation Policy</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><div className="form-group" style={{marginBottom:0}}><label className="form-label">Strategy</label><select value={content?.truncationPolicy?.global?.strategy||'ellipsis'} onChange={e=>updateContent('truncationPolicy',{...(content.truncationPolicy||{}),global:{...((content.truncationPolicy||{}).global||{}),strategy:e.target.value}})}><option value="ellipsis">ellipsis</option><option value="clamp">clamp</option><option value="wrap">wrap</option></select></div><div className="form-group" style={{marginBottom:0}}><label className="form-label">Clamp lines</label><input value={content?.truncationPolicy?.global?.maxLines||''} onChange={e=>updateContent('truncationPolicy',{...(content.truncationPolicy||{}),global:{...((content.truncationPolicy||{}).global||{}),maxLines:Number(e.target.value)||null}})} placeholder="auto"/></div></div><div className="form-group" style={{marginTop:8}}><label className="form-label">Priority fields (comma-sep)</label><input value={(content?.truncationPolicy?.priorityFields||[]).join(', ')} onChange={e=>updateContent('truncationPolicy',{...(content.truncationPolicy||{}),priorityFields:e.target.value.split(',').map(v=>v.trim()).filter(Boolean)})}/></div>{fallbackTruncation&&<div style={{fontSize:10,color:'#FFB3B3',marginTop:6}}>Fallback truncation active. Auto-fix: {truncationSuggestions[0]||'Shorten copy.'}</div>}</div><div className="editor-section"><div className="editor-section-title">Metadata</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>{[['Issue #','issueNum'],['Date','date'],['Desk','desk'],['Volume','volume']].map(([l,k])=>(<div key={k} className="form-group" style={{marginBottom:0}}><label className="form-label">{l}</label><input value={content[k]} onChange={e=>updateContent(k,e.target.value)}/></div>))}</div><div className="form-group" style={{marginTop:8}}><label className="form-label">Topic Tag</label><input value={content.topicTag} onChange={e=>{const next=normalizeContentTaxonomy({...content,topicTag:e.target.value});updateContent('topicTag',next.content.topicTag);updateContent('slug',next.content.slug);if(next.compliance.autoCorrected.length)showToast('Topic/slug normalized on input.','info');if(next.compliance.hasInvalid)showToast(next.compliance.invalid.join(' '),'error');}}/></div>
      <div className="form-group">
...
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><div className="form-group" style={{marginBottom:0}}><label className="form-label">@X</label><input value={content.socialX||''} onChange={e=>updateContent('socialX',e.target.value)}/></div><div className="form-group" style={{marginBottom:0}}><label className="form-label">@Sub</label><input value={content.socialSub||''} onChange={e=>updateContent('socialSub',e.target.value)}/></div></div></div>
          <div className="editor-section"><div className="editor-section-title">Editorial</div><div className="form-group"><label className="form-label">Title {overflowMeta?.actions?.title?.length? '⚠ truncating':''}</label><textarea value={content.title} onChange={e=>updateContent('title',e.target.value)} rows={2}/><div className={`char-count ${content.title.length>60?'warn':''} ${content.title.length>80?'over':''}`}>{content.title.length}/60</div></div><div className="form-group"><label className="form-label">Deck {overflowMeta?.actions?.deck?.length? '⚠ truncating':''}</label><textarea value={content.deck} onChange={e=>updateContent('deck',e.target.value)} rows={2}/><div className={`char-count ${content.deck.length>120?'warn':''}`}>{content.deck.length}/120</div></div><div className="form-group"><label className="form-label">Body {overflowMeta?.actions?.body?.length? '⚠ truncating':''}</label><textarea value={content.body} onChange={e=>updateContent('body',e.target.value)} rows={4}/><div className={`char-count ${content.body.length>400?'warn':''}`}>{content.body.length}/400</div></div></div>
          <div className="editor-section">
