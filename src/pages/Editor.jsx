...
import { parseImportPayload, normalizeEditorState } from '../domain/import';
...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const restoreAutosave=()=>{const d=normalizeEditorState(autosaveDataRef.current,{content:DEFAULT_CONTENT});if(d){d.frameId&&setFrameId(d.frameId);d.theme&&(setTheme(d.theme),setActiveTheme(d.theme));d.content&&resetContent(d.content);d.overrides&&setOverrides(d.overrides);d.aspectRatio&&setAspectRatio(d.aspectRatio);d.bgGradient&&setBgGradient(d.bgGradient);d.patternOverlay&&setPatternOverlay(d.patternOverlay);showToast('Restored');}setShowAutosavePrompt(false);};
  ...
  const loadSave=s=>{const n=normalizeEditorState(s,{content:DEFAULT_CONTENT});n.frameId&&setFrameId(n.frameId);n.theme&&setTheme(n.theme);n.content&&resetContent(n.content);n.overrides&&setOverrides(n.overrides);n.aspectRatio&&setAspectRatio(n.aspectRatio);n.bgGradient&&setBgGradient(n.bgGradient);n.patternOverlay&&setPatternOverlay(n.patternOverlay);setShowLoadModal(false);showToast(`Loaded`);};
  ...
  const importJSON=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=ev=>{const raw=parseImportPayload(ev.target.result);if(!raw){showToast('Invalid JSON','error');return;}const d=normalizeEditorState(raw,{content:DEFAULT_CONTENT});d.frameId&&setFrameId(d.frameId);d.theme&&(setTheme(d.theme),setActiveTheme(d.theme));d.content&&resetContent(d.content);d.overrides&&setOverrides(d.overrides);d.aspectRatio&&setAspectRatio(d.aspectRatio);d.bgGradient&&setBgGradient(d.bgGradient);d.patternOverlay&&setPatternOverlay(d.patternOverlay);showToast('Imported');};r.readAsText(f);e.target.value='';};
