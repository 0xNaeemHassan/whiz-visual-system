...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const restoreAutosave=()=>{const d=autosaveDataRef.current;if(d){const v=validateEditorState(d);if(!v.valid){showToast(`Autosave validation failed (${v.codes.join(', ')})`,'error');setShowAutosavePrompt(false);return;}d.frameId&&setFrameId(d.frameId);d.theme&&(setTheme(d.theme),setActiveTheme(d.theme));d.content&&resetContent(d.content);d.overrides&&setOverrides(d.overrides);d.aspectRatio&&setAspectRatio(d.aspectRatio);d.bgGradient&&setBgGradient(d.bgGradient);d.patternOverlay&&setPatternOverlay(d.patternOverlay);showToast('Restored');}setShowAutosavePrompt(false);};
  ...
  const loadSave=s=>{const v=validateEditorState(s);if(!v.valid){showToast(`Load failed (${v.codes.join(', ')})`,'error');return;}setFrameId(s.frameId);setTheme(s.theme);resetContent(s.content);s.overrides&&setOverrides(s.overrides);s.aspectRatio&&setAspectRatio(s.aspectRatio);s.bgGradient&&setBgGradient(s.bgGradient);s.patternOverlay&&setPatternOverlay(s.patternOverlay);setShowLoadModal(false);showToast(`Loaded`);};
  ...
  const importJSON=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=ev=>{try{const d=JSON.parse(ev.target.result);const v=validateEditorState(d);if(!v.valid){showToast(`Import failed (${v.codes.join(', ')})`,'error');return;}d.frameId&&setFrameId(d.frameId);d.theme&&(setTheme(d.theme),setActiveTheme(d.theme));d.content&&resetContent(d.content);d.overrides&&setOverrides(d.overrides);d.aspectRatio&&setAspectRatio(d.aspectRatio);d.bgGradient&&setBgGradient(d.bgGradient);d.patternOverlay&&setPatternOverlay(d.patternOverlay);showToast('Imported');}catch(err){showToast('Invalid JSON','error');}};r.readAsText(f);e.target.value='';};
  ...
  const applyTemplate=t=>{const next={content:{...DEFAULT_CONTENT,...t.content},overrides,uploadedImages};const v=validateEditorState(next);if(!v.valid){showToast(`Template blocked (${v.codes.join(', ')})`,'error');return;}resetContent(next.content);showToast(`Template: ${t.name}`);};
