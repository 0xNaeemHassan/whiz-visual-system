...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const [correctionDraft, setCorrectionDraft] = useState({ issueNum: '', correctedFields: '', rationale: '', supersedePrior: false });
  const [correctionLog, setCorrectionLog] = useLocalStorage('whiz-corrections', {});
  const [artifactSnapshots, setArtifactSnapshots] = useLocalStorage('whiz-export-snapshots', {});
  const currentArtifactId = buildArtifactId({ issueNum: content?.issueNum, slug: content?.slug });
  const correctionCount = correctionLog[currentArtifactId]?.length || 0;
  ...
  const createExportSnapshot = (normalizedContent) => {
    const artifactId = buildArtifactId({ issueNum: content.issueNum, slug: normalizedContent.slug });
    const snapshot = Object.freeze({
      artifactId,
      issueNum: String(content.issueNum || ''),
      manifest: { frameId, themeId: theme.id, slug: normalizedContent.slug || '', topicTag: normalizedContent.topicTag || '' },
      exportedAt: new Date().toISOString(),
    });
    setArtifactSnapshots((prev) => ({ ...prev, [artifactId]: prev[artifactId] || [], [artifactId]: [...(prev[artifactId] || []), snapshot] }));
    return { artifactId, snapshot };
  };
  const createCorrectionRecord = ({ manifestRef, artifactId }) => {
    const issueNum = correctionDraft.issueNum || content.issueNum;
    const correctedFields = correctionDraft.correctedFields.split(',').map((f) => f.trim()).filter(Boolean);
    if (!issueNum || correctedFields.length === 0 || !correctionDraft.rationale.trim()) {
      showToast('Correction requires issue #, corrected fields, and rationale.', 'error');
      return null;
    }
    const record = {
      artifactId,
      issueNum: String(issueNum),
      correctionNumber: (correctionLog[artifactId]?.length || 0) + 1,
      correctedFields,
      rationale: correctionDraft.rationale.trim(),
      createdAt: new Date().toISOString(),
      supersedesPriorExport: !!correctionDraft.supersedePrior,
      originalManifestRef: manifestRef,
    };
    setCorrectionLog((prev) => ({ ...prev, [artifactId]: [...(prev[artifactId] || []), record] }));
    const sidecar = {
      type: 'whiz.correction.sidecar',
      artifactId,
      backlinks: { originalManifest: manifestRef },
      correction: record,
    };
    downloadFile(`${artifactId.replaceAll(':', '_')}.correction.json`, new Blob([JSON.stringify(sidecar, null, 2)], { type: 'application/json' }));
    track(TELEMETRY_EVENTS.EXPORT_CORRECTION_CREATED, {
      artifactId,
      issueNum: String(issueNum),
      correctedFieldCount: correctedFields.length,
      supersedesPriorExport: !!correctionDraft.supersedePrior,
    });
    showToast('Correction recorded + sidecar exported.');
    return record;
  };
