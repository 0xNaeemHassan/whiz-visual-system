...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const pushAuditEvent = useCallback(({ actor = activeRole || 'editor', fieldPath, oldValue, newValue, section = 'general', action = 'update', context = {} }) => {
    const event = { id:`ae_${Date.now()}_${Math.random().toString(36).slice(2,8)}`, actor, timestamp:new Date().toISOString(), fieldPath, oldValueHash:simpleHash(oldValue), newValueHash:simpleHash(newValue), section, action, context };
    setAuditTrail((prev) => [...prev.slice(-799), event]);
  }, [activeRole, setAuditTrail]);
  const buildSave=()=>buildFrameSave({frameId,theme,content,overrides,aspectRatio,bgGradient,patternOverlay,workflowPhase,phaseChecklist,sectionLocks,auditTrail});
  const doSave=()=>{const n=saveName.trim()||`${content.topicTag} \u2014 ${new Date().toLocaleDateString()}`;pushAuditEvent({fieldPath:'save',oldValue:null,newValue:n,section:'publish',action:'save',context:{workflowPhase}});setSaves(p=>[...p,{id:`s_${Date.now()}`,title:n,...buildSave()}]);setShowSaveModal(false);setSaveName('');showToast(`Saved "${n}"`);};
  ...
  useEffect(()=>{['stats','tableRows','metricProvenance'].forEach((key)=>{const prev=lastContentRef.current?.[key];const cur=content?.[key];if(JSON.stringify(prev)!==JSON.stringify(cur)){pushAuditEvent({fieldPath:`content.${key}`,oldValue:prev,newValue:cur,section:key==='tableRows'?'tables':key==='metricProvenance'?'provenance':'stats'});}});lastContentRef.current=content;},[content,pushAuditEvent]);
