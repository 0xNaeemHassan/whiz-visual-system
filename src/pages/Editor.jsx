...
import { normalizeNumericFields } from '../utils/numericNormalization';
...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const [normalizationAudit, setNormalizationAudit] = useState([]);
  const buildSave=(saveMetadata={})=>buildFrameSave({frameId,theme,content,overrides,aspectRatio,bgGradient,patternOverlay,saveMetadata});
  const doSave=()=>{const n=saveName.trim()||`${content.topicTag} \u2014 ${new Date().toLocaleDateString()}`;setSaves(p=>[...p,{id:`s_${Date.now()}`,title:n,...buildSave(metadata)}]);setShowSaveModal(false);setSaveName('');showToast(`Saved "${n}"`);};
