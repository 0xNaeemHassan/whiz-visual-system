...
{rowDeltaFlags.length>0&&<div className="editor-section"><div className="editor-section-title">Anomalous Rows</div>{rowDeltaFlags.map((flag)=><div key={`anomaly-${flag.rowIndex}`} style={{fontSize:11,padding:'7px 8px',borderRadius:6,border:'1px solid rgba(229,178,58,0.45)',background:'rgba(229,178,58,0.1)',marginBottom:6}}><div>Row {flag.rowIndex+1}: {flag.explanation}</div><div style={{opacity:0.8}}>Action: verify source/provenance, then acknowledge to permit strict export.</div><button className="btn btn-ghost btn-sm" onClick={()=>setRowDeltaFlags((prev)=>prev.map((p)=>p.rowIndex===flag.rowIndex?{...p,acknowledged:true}:p))}>{flag.acknowledged?'Acknowledged':'Acknowledge'}</button></div>)}</div>}
          <div className="editor-section">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
