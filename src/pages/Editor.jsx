...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const [sectionLocks, setSectionLocks] = useState(DEFAULT_SECTION_LOCKS);
  ...
  const buildSave=()=>buildFrameSave({frameId,theme,content,overrides,aspectRatio,bgGradient,patternOverlay,sectionLocks});
...
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><div className="form-group" style={{marginBottom:0}}><label className="form-label">@X</label><input value={content.socialX||''} onChange={e=>updateContent('socialX',e.target.value)}/></div><div className="form-group" style={{marginBottom:0}}><label className="form-label">@Sub</label><input value={content.socialSub||''} onChange={e=>updateContent('socialSub',e.target.value)}/></div></div></div>
          <div className="editor-section" style={{opacity:isSectionLocked('headline')?0.55:1}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div className="editor-section-title">Editorial</div><button className="btn btn-ghost btn-sm" onClick={()=>toggleSectionLock('headline')}>{isSectionLocked('headline')?'🔒 Locked':'🔓 Unlock'}</button></div><div className="form-group"><label className="form-label">Title</label><textarea disabled={isSectionLocked('headline')} value={content.title} onChange={e=>updateContent('title',e.target.value)} rows={2}/><div className={`char-count ${content.title.length>60?'warn':''} ${content.title.length>80?'over':''}`}>{content.title.length}/60</div></div><div className="form-group"><label className="form-label">Deck</label><textarea disabled={isSectionLocked('headline')} value={content.deck} onChange={e=>updateContent('deck',e.target.value)} rows={2}/><div className={`char-count ${content.deck.length>120?'warn':''}`}>{content.deck.length}/120</div></div><div className="form-group"><label className="form-label">Body</label><textarea disabled={isSectionLocked('body')} value={content.body} onChange={e=>updateContent('body',e.target.value)} rows={4}/><div className={`char-count ${content.body.length>400?'warn':''}`}>{content.body.length}/400</div></div></div>
          <div className="editor-section">
