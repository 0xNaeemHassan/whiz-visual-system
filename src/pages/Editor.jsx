...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const logConfirmationEvent = useCallback((actionId, label, outcome) => {
    setActivityLog((prev) => [{ id: `confirm-${Date.now()}-${Math.random().toString(16).slice(2)}`, ts: Date.now(), actionId, label, outcome }, ...prev].slice(0, 200));
  }, []);
  const requestConfirmation = useCallback((options) => new Promise((resolve) => {
    const skipKey = options?.skipKey;
    if (skipKey && sessionConfirmSkips[skipKey]) {
      logConfirmationEvent(options.actionId || skipKey, options.title || skipKey, 'auto-confirmed');
      resolve(true);
      return;
    }
    setConfirmState({
      actionId: options.actionId || options.title || 'confirm-action',
      ...
      skipKey: options.skipKey || null,
      skipChecked: false,
      resolve,
    });
  }), [logConfirmationEvent, sessionConfirmSkips]);
  ...
  return(
    <div className="editor-wrap" ref={editorRootRef}>
      ...
      <div className={`editor-left ${mobileTab==='frame'?'mob-active':''}`}>
        ...
        <div className="editor-section" style={{flex:1}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}><span className="editor-section-title" style={{marginBottom:0}}>Saves ({saves.length})</span><button className="btn btn-primary btn-sm" onClick={()=>setShowSaveModal(true)}>Save</button></div>{saves.length===0?<div style={{fontSize:11,color:'var(--dim)',textAlign:'center',padding:'16px 0'}}>No saves yet</div>:<div className="saves-list">{saves.slice().reverse().map(s=>(<div key={s.id} className="save-item" onClick={()=>loadSave(s)}><div style={{width:24,height:24,borderRadius:4,background:s.theme?.base||'var(--bg-3)',border:`2px solid ${s.theme?.accent||'var(--border)'}`,flexShrink:0}}/><div className="save-item-info"><div className="save-item-name">{s.title}</div><div className="save-item-meta">#{s.frameId} · {new Date(s.savedAt).toLocaleDateString()}</div></div><button className="btn btn-ghost btn-sm" onClick={e=>{e.stopPropagation();confirmDel(s.id);}}>✕</button></div>))}</div>}</div>
      </div>
      {/* CENTER */}
