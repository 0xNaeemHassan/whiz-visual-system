...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const applyValidImportedRows = () => {
    ...
    let overrideReasonCode = '';
    if (tableImportReport.invalidRows?.length) {
      const reasonInput = window.prompt(`This import has parser/normalizer warnings. Enter override reason code: ${OVERRIDE_REASON_CODES.join(', ')}`)?.trim();
      if (!reasonInput || !OVERRIDE_REASON_CODES.includes(reasonInput)) {
        showToast('Import cancelled: valid override reason code required.', 'warning');
        return;
      }
      overrideReasonCode = reasonInput;
    }
    updateContent('tableRows', [...(content.tableRows || []), ...tableImportReport.validRows.map((row)=>({
      ...row,
      provenance: createDefaultProvenance(),
      overrideAudit: createOverrideAuditEntry({ reasonCode: overrideReasonCode, warningSource: tableImportReport.invalidRows?.length ? 'parser-normalizer-warning' : '' }),
    }))]);
