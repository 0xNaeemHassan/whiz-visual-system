...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  useEffect(()=>{(async()=>{try{const r=localStorage.getItem('whiz-autosave');if(r){const d=JSON.parse(r);const payload=d.payload||d;const fastHash=d.fastHash||d.checksum||null;const expectedFastHash=computeAutosaveChecksum(payload);const expectedIntegrityDigest=await buildIntegrityDigest(stableStringify(payload));const hasFastHashIssue=Boolean(fastHash&&fastHash!==expectedFastHash);const hasIntegrityIssue=Boolean(d.integrityDigest&&expectedIntegrityDigest&&d.integrityDigest!==expectedIntegrityDigest);const hasRecoveryMarker=Boolean(d.interrupted||d.sessionEndedAt==null);if(payload.savedAt&&Date.now()-payload.savedAt<86400000){autosaveDataRef.current={...payload,revisionId:d.revisionId||`rev-${payload.savedAt}`,savedAt:payload.savedAt,createdAt:d.createdAt||payload.savedAt,updatedAt:d.updatedAt||payload.savedAt};setAutosaveRecoveryPlan({hasIntegrityIssue:hasFastHashIssue||hasIntegrityIssue,hasRecoveryMarker,conflicts:{frameId:payload.frameId!==frameId,theme:JSON.stringify(payload.theme)!==JSON.stringify(theme),overrides:JSON.stringify(payload.overrides)!==JSON.stringify(overrides)}});setMergeSelection({ frameId: payload.frameId!==frameId, theme: JSON.stringify(payload.theme)!==JSON.stringify(theme), overrides: JSON.stringify(payload.overrides)!==JSON.stringify(overrides) });setShowAutosavePrompt(true);}}}catch(e){}})();},[]);
  ...
  const exportJSON=async()=>{if(!validateExportTypography())return;if(!validateCriticalExportFields('json').valid)return;if(!ensureActionAllowed('export'))return;const normalized=await runNormalizationPreflight();if(!normalized)return;const payload={...buildSave(),content:{...content,...normalized.normalizedContent}};const canonicalPayload=stableStringify(payload);const exportPackage={...payload,exportMetadata:{fastHash:computeAutosaveChecksum(payload),integrityDigest:await buildIntegrityDigest(canonicalPayload),integrityAlgorithm:'SHA-256',fastHashAlgorithm:'FNV-1a'}};const d=JSON.stringify(exportPackage,null,2);const b=new Blob([d],{type:'application/json'});withManagedDownload(b,`${normalized.normalizedContent.slug||'whiz_export'}.json`);showToast('JSON exported');};
  ...
  const importJSON=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=async(ev)=>{try{const raw=JSON.parse(ev.target.result);const payload={...raw};delete payload.exportMetadata;const canonicalPayload=stableStringify(payload);const expectedFastHash=computeAutosaveChecksum(payload);const expectedIntegrityDigest=await buildIntegrityDigest(canonicalPayload);const fastHash=raw?.exportMetadata?.fastHash;const integrityDigest=raw?.exportMetadata?.integrityDigest;if(fastHash&&fastHash!==expectedFastHash)throw new Error('Import rejected: fastHash mismatch.');if(integrityDigest&&expectedIntegrityDigest&&integrityDigest!==expectedIntegrityDigest)throw new Error('Import rejected: integrityDigest mismatch.');const d=parseImportedState(raw);d.frameId&&setFrameId(d.frameId);d.theme&&(setTheme(d.theme),setActiveTheme(d.theme));d.content&&resetContent(normalizeContentWithProvenance(d.content));d.overrides&&setOverrides(d.overrides);d.aspectRatio&&setAspectRatio(d.aspectRatio);d.bgGradient&&setBgGradient(d.bgGradient);d.patternOverlay&&setPatternOverlay(d.patternOverlay);setWorkflowPhase(WORKFLOW_PHASES.includes(d.workflowPhase)?d.workflowPhase:'draft');setPhaseChecklist(d.phaseChecklist||{draftAt:d.savedAt||Date.now(),reviewAt:null,publishReadyAt:null,lastTransitionAt:d.savedAt||Date.now()});showToast('Imported');}catch(err){showToast(err?.message||'Invalid JSON','error');}};r.readAsText(f);e.target.value='';};
  ...
  useEffect(()=>{if(!isActive)return;const t=setTimeout(async()=>{try{const payload={frameId,theme,content,overrides,aspectRatio,bgGradient,patternOverlay,savedAt:Date.now()};localStorage.setItem('whiz-autosave',JSON.stringify({payload,fastHash:computeAutosaveChecksum(payload),integrityDigest:await buildIntegrityDigest(stableStringify(payload))}));}catch(e){}},3000);return()=>clearTimeout(t);},[isActive,content,overrides,frameId,theme,aspectRatio,bgGradient,patternOverlay]);
