...
import { DOMAIN_ROLES, ROLE_ORDER, ROLE_LABELS, canRole, getRoleDisableReason } from '../domain/roleConfig';
...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const [activeRole, setActiveRole] = useLocalStorage('whiz-editor-role', DOMAIN_ROLES.PUBLISHER);
...
<div className="editor-section"><div className="editor-section-title">Table</div><div className="form-group"><label className="form-label">Headers (comma-sep)</label><input value={(content.tableHeaders||[]).join(', ')} onChange={e=>updateContent('tableHeaders',e.target.value.split(',').map(s=>s.trim()).filter(Boolean))}/></div>{(content.tableRows||[]).map((row,i)=>(<DragItem key={`tr-${i}`} index={i} id={`tr-${i}`} onMove={(f,t)=>{const a=[...content.tableRows];const[it]=a.splice(f,1);a.splice(t,0,it);updateContent('tableRows',a);}}><div style={{display:'grid',gridTemplateColumns:`auto repeat(${Object.keys(row).length},1fr) auto`,gap:3,marginBottom:3,alignItems:'center'}}><span style={{color:'var(--dim)',fontSize:10,cursor:'grab'}}>⋮⋮</span>{Object.keys(row).map(k=>(<input key={k} value={row[k]} style={{fontSize:10,padding:'5px 6px'}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],[k]:e.target.value};updateContent('tableRows',a);}}/>))}<button className="btn btn-danger btn-sm" style={{padding:'0 7px',height:30}} onClick={()=>updateContent('tableRows',content.tableRows.filter((_,r)=>r!==i))}>✕</button></div></DragItem>))}<button className="btn btn-secondary btn-sm w-full" style={{marginTop:6}} onClick={()=>{const c=(content.tableHeaders||[]).length||5;const r={};for(let j=1;j<=c;j++)r[`col${j}`]='';updateContent('tableRows',[...(content.tableRows||[]),r]);}}>+ Row</button></div>
          ...
          <div className="editor-section"><div className="editor-section-title">Role</div><div style={{display:'flex',gap:6,marginBottom:8}}>{ROLE_ORDER.map((role)=>{const active=role===activeRole;return <button key={role} className={`btn btn-sm ${active?'btn-primary':'btn-ghost'}`} style={{flex:1}} onClick={()=>setActiveRole(role)}>{ROLE_LABELS[role]}</button>;})}</div><div style={{fontSize:10,color:'var(--dim)'}}>Current role: <strong style={{color:'var(--text)'}}>{ROLE_LABELS[activeRole]}</strong></div></div><div className="editor-section"><div className="editor-section-title">Frame Warnings</div>{activeFramePitfalls.length===0?<div style={{fontSize:11,color:'var(--dim)'}}>No known pitfalls for this frame.</div>:<div style={{display:'flex',flexDirection:'column',gap:6}}>{activeFramePitfalls.map(p=>(<div key={p.id} style={{fontSize:11,padding:'7px 8px',borderRadius:6,border:`1px solid ${p.severity==='high'?'rgba(235,87,87,0.45)':'rgba(229,178,58,0.45)'}`,background:p.severity==='high'?'rgba(235,87,87,0.08)':'rgba(229,178,58,0.1)'}}><div style={{fontFamily:'var(--font-m)',fontSize:9,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:4}}>{p.severity==='high'?'Blocking':'Non-blocking'}</div><div>{p.warning}</div><div style={{opacity:0.75,marginTop:4}}>Hint: {p.triggerHint}</div></div>))}</div>}<div style={{marginTop:8,fontSize:10,color:'var(--dim)'}}>Readiness checklist: {rolePermissions.canRunReadiness?'enabled':'disabled'}{!rolePermissions.canRunReadiness?` — ${roleReasons.readiness}`:''}</div></div><div className="editor-section"><div className="editor-section-title">Export</div><div style={{fontFamily:'var(--font-m)',fontSize:9,color:'var(--dim)',marginBottom:8,lineHeight:1.7}}>⌘S Save · ⌘Z Undo · ⌘⇧Z Redo</div><div style={{display:'flex',flexDirection:'column',gap:8}}><div style={{display:'flex',gap:6}}><button className="btn btn-primary btn-sm" style={{flex:1}} onClick={exportPNG} disabled={exporting||!rolePermissions.canExportAssets} title={!rolePermissions.canExportAssets?roleReasons.exportAssets:''}>{exporting?'…':`PNG`}</button><button className="btn btn-secondary btn-sm editor-action-btn" onClick={exportWebP} disabled={exporting||!rolePermissions.canExportAssets} title={!rolePermissions.canExportAssets?roleReasons.exportAssets:''}>WebP</button></div><div style={{display:'flex',gap:6}}><button className="btn btn-ghost btn-sm" style={{flex:1}} onClick={exportHTML} disabled={!rolePermissions.canExportAssets} title={!rolePermissions.canExportAssets?roleReasons.exportAssets:''}>HTML</button><button className="btn btn-ghost btn-sm" style={{flex:1}} onClick={exportJSON} disabled={!rolePermissions.canPublish} title={!rolePermissions.canPublish?roleReasons.publish:''}>JSON</button></div><label className="import-label">↑ Import <input type="file" accept=".json" onChange={importJSON}/></label><button className="btn btn-secondary w-full" onClick={()=>setShowSaveModal(true)}>Save</button><button className="btn btn-ghost w-full" onClick={()=>{const n=`${content.title||'Frame'} (copy)`;setSaves(p=>[...p,{id:`s_${Date.now()}`,title:n,...buildSave()}]);showToast('Duplicated');}}>Duplicate</button>{saves.length>0&&<button className="btn btn-ghost w-full" onClick={()=>{setSaveSearch('');setShowLoadModal(true);}}>Load ({saves.length})</button>}</div></div>
        </>)}
      </div>
      {/* MODALS */}
