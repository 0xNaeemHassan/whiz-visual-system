...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const applyValidImportedRows = () => {
    ...
    const appendedRows = [...(content.tableRows || []), ...tableImportReport.validRows.map((row)=>({ ...row, provenance: createDefaultProvenance() }))];
    updateContent('tableRows', normalizeNumericFields({ ...content, tableRows: appendedRows }).content.tableRows);
