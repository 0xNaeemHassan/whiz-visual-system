...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const importJSON=e=>{const f=e.target.files?.[0];if(!f)return;if(!checkInputRateLimit(importWindowRef,INPUT_GUARDRAILS.jsonImportMaxEvents,'json_import')){e.target.value='';return;}if(f.size>INPUT_GUARDRAILS.jsonImportMaxBytes){trackInputGuardrailBlocked({inputType:'json_import',reason:'payload_size',maxAllowed:INPUT_GUARDRAILS.jsonImportMaxBytes});showToast(`Import rejected: JSON file is too large. Keep it under ${Math.round(INPUT_GUARDRAILS.jsonImportMaxBytes/1024/1024)}MB or trim large arrays.`,'error');e.target.value='';return;}const r=new FileReader();r.onload=ev=>{try{const d=parseImportedState(JSON.parse(ev.target.result));const importedRows=Array.isArray(d?.content?.tableRows)?d.content.tableRows.length:0;const importedEvents=Array.isArray(d?.content?.timelineEvents)?d.content.timelineEvents.length:0;const importedMax=Math.max(importedRows,importedEvents);if(importedMax>INPUT_GUARDRAILS.tablePasteMaxRows){trackInputGuardrailBlocked({inputType:'json_import',reason:'item_count',droppedCount:importedMax-INPUT_GUARDRAILS.tablePasteMaxRows,maxAllowed:INPUT_GUARDRAILS.tablePasteMaxRows});showToast(`Import rejected: too many rows/events (${importedMax}). Limit is ${INPUT_GUARDRAILS.tablePasteMaxRows}. Split data and retry.`,'error');return;}d.frameId&&setFrameId(d.frameId);d.theme&&(setTheme(d.theme),setActiveTheme(d.theme));d.content&&resetContent(normalizeContentWithProvenance(d.content));d.overrides&&setOverrides(d.overrides);d.aspectRatio&&setAspectRatio(d.aspectRatio);d.bgGradient&&setBgGradient(d.bgGradient);d.patternOverlay&&setPatternOverlay(d.patternOverlay);setWorkflowPhase(WORKFLOW_PHASES.includes(d.workflowPhase)?d.workflowPhase:'draft');setPhaseChecklist(d.phaseChecklist||{draftAt:d.savedAt||Date.now(),reviewAt:null,publishReadyAt:null,lastTransitionAt:d.savedAt||Date.now()});showToast('Imported');}catch(err){showToast('Invalid JSON','error');}};r.readAsText(f);e.target.value='';};
