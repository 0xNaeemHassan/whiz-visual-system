...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const doSave=()=>{const snapshot=buildSave();const n=saveName.trim()||`${content.topicTag} — ${new Date().toLocaleDateString()}`;const tagNormalization=normalizeCanonicalTagList(saveTagsInput);const nextSave={id:`s_${Date.now()}`,title:n,...snapshot,tags:tagNormalization.valid,folder:saveFolder.trim(),...(saveStatus.trim()?{status:saveStatus.trim()}:{}),auditTrailSnapshot:[...auditTrail]};setSaves(p=>[...p,nextSave]);if(tagNormalization.invalid.length){showToast(`Ignored non-canonical tags: ${tagNormalization.invalid.join(', ')}`,'warning');}if(tagNormalization.remapped.length){showToast('Tag aliases/deprecations auto-mapped to canonical values.','info');}appendAuditEvent({action:'save',section:'editor',fieldPath:'save',oldValueHash:'n/a',newValueHash:hashValue(snapshot),context:{saveName:n,saveCount:saves.length+1,tags:tagNormalization.valid}});setShowSaveModal(false);setSaveName('');setSaveTagsInput('');setSaveFolder('');setSaveStatus('');showToast(`Saved "${n}"`);};
