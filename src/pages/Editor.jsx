...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const [showMobileCommandSheet, setShowMobileCommandSheet] = useState(false);
  const [mobileQaChecks, setMobileQaChecks] = useState({
    quickEdit: false, validate: false, publish: false, proRows: false,
  });
  ...
  const primaryValidationIssue = editorValidation?.issues?.[0] || complianceIssues?.[0] || null;
  const validationIssueCount = (editorValidation?.issues?.length || 0) + complianceIssues.length;
...
<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}><div className="form-group" style={{marginBottom:0}}><label className="form-label">@X</label><input value={content.socialX||''} onChange={e=>updateContent('socialX',e.target.value)}/></div><div className="form-group" style={{marginBottom:0}}><label className="form-label">@Sub</label><input value={content.socialSub||''} onChange={e=>updateContent('socialSub',e.target.value)}/></div></div></div>
          ...
          <div className="editor-section"><div className="editor-section-title">Stats</div><div role="list">{content.stats.map((s,i)=>(<DragItem key={`s-${i}`} index={i} id={`s-${i}`} onMove={(f,t)=>{const a=[...content.stats];const[it]=a.splice(f,1);a.splice(t,0,it);updateContent('stats',a);}}><div className="touch-row-editor touch-row-editor--stats"><button className="touch-drag-handle" aria-label="Drag stat row">⋮⋮</button><input value={s.label} placeholder="LABEL" style={{fontSize:11}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],label:e.target.value};updateContent('stats',a);}}/><input value={s.value} placeholder="VALUE" style={{fontSize:11}} onChange={e=>{const a=[...content.stats];a[i]={...a[i],value:e.target.value};updateContent('stats',a);}}/><button className="btn btn-danger btn-sm touch-row-menu" onClick={()=>updateContent('stats',content.stats.filter((_,r)=>r!==i))}>Row ✕</button></div></DragItem>))}</div><button className="btn btn-secondary btn-sm w-full" style={{marginTop:4}} onClick={()=>updateContent('stats',[...content.stats,{label:'',value:''}])}>+ Stat</button></div>
...
<div className="editor-section"><div className="editor-section-title">Table</div><div className="form-group"><label className="form-label">Headers (comma-sep)</label><input value={(content.tableHeaders||[]).join(', ')} onChange={e=>updateContent('tableHeaders',e.target.value.split(',').map(s=>s.trim()).filter(Boolean))}/></div>{(content.tableRows||[]).map((row,i)=>(<DragItem key={`tr-${i}`} index={i} id={`tr-${i}`} onMove={(f,t)=>{const a=[...content.tableRows];const[it]=a.splice(f,1);a.splice(t,0,it);updateContent('tableRows',a);}}><div className="touch-row-editor touch-row-editor--table" style={{gridTemplateColumns:`auto repeat(${Object.keys(row).length},1fr) auto`}}><button className="touch-drag-handle" aria-label="Drag table row">⋮⋮</button>{Object.keys(row).map(k=>(<input key={k} value={row[k]} style={{fontSize:10,padding:'5px 6px'}} onChange={e=>{const a=[...content.tableRows];a[i]={...a[i],[k]:e.target.value};updateContent('tableRows',a);}}/>))}<button className="btn btn-danger btn-sm touch-row-menu" onClick={()=>updateContent('tableRows',content.tableRows.filter((_,r)=>r!==i))}>Row ✕</button></div></DragItem>))}<button className="btn btn-secondary btn-sm w-full" style={{marginTop:6}} onClick={()=>{const c=(content.tableHeaders||[]).length||5;const r={};for(let j=1;j<=c;j++)r[`col${j}`]='';updateContent('tableRows',[...(content.tableRows||[]),r]);}}>+ Row</button></div>
          <div className="editor-section">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
          ...
          <div className="editor-section">
            ...
            {(content.timelineEvents||[]).map((ev,i)=>(
              <div key={i} className="touch-row-editor touch-row-editor--timeline" style={{gridTemplateColumns:'auto 80px 1fr 1fr auto'}}>
                <button className="touch-drag-handle" aria-label="Drag timeline row">⋮⋮</button>
                ...
                <button className="btn btn-danger btn-sm touch-row-menu" onClick={()=>updateContent('timelineEvents',(content.timelineEvents||[]).filter((_,r)=>r!==i))}>Row ✕</button>
              </div>
            ))}
