...
import { evaluateConstraintRegistry, getConstraintRuleIds } from '../domain/constraintRegistry';
...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const runTableImportValidation = () => {
    ...
    const constraintInvalidRows = [];
    validRows.forEach((row, idx) => {
      const violations = evaluateConstraintRegistry({ frameId, content: { ...content, tableRows: [row] } });
      if (violations.length) {
        constraintInvalidRows.push({ rowIndex: idx, row, errors: violations.map((v) => ({ column: 'row', message: `${v.ruleId}: ${v.message}`, ruleId: v.ruleId })) });
      }
    });
    const finalInvalidRows = [...invalidRows, ...constraintInvalidRows];
    const finalValidRows = validRows.filter((_, idx) => !constraintInvalidRows.some((entry) => entry.rowIndex === idx));
    const report = { ...baseReport, validRows: finalValidRows, invalidRows: finalInvalidRows };
