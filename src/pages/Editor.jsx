...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const buildExportSummary=(normalizedContent)=>generateExportSummary({frame:activeFrame,content:{...content,...normalizedContent,trustLevel},complianceIssues,validationWarnings:editorValidation.warnings});
  ...
  return(
    <div className="editor-wrap" ref={editorRootRef}>
      ...
      <div className={`editor-center ${mobileTab==='preview'?'mob-active':''}`} ref={centerRef}>
        ...
        <div style={{position:'absolute',top:10,left:10,fontFamily:'var(--font-m)',fontSize:9,color:'var(--dim)',background:'rgba(0,0,0,0.6)',padding:'4px 8px',borderRadius:'var(--r)',backdropFilter:'blur(4px)'}}>{String(frameId).padStart(2,'0')} — {selectedFrame.name} · {aspectRatio.w}×{aspectRatio.h}</div><div style={{position:'absolute',top:10,right:12,fontFamily:'var(--font-m)',fontSize:10,color:trustTone.fg,background:trustTone.bg,padding:'5px 8px',borderRadius:6,border:`1px solid ${trustTone.border}`}}>Trust: {trustLevel}</div>
