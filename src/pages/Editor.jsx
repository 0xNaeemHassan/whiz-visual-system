...
import { buildMutationDispatcher } from './EditorMutations.jsx';
...
const DEFAULT_CONTENT = {
  ...
  evidenceLedger:createDefaultEvidenceLedger('001'),
};
/** @type {StyleOverrides} */
...
const buildArtifactId = ({ issueNum, slug }) => `artifact:${String(issueNum || '000')}:${String(slug || 'unsluggified')}`;

const PROVENANCE_CONFIDENCE_OPTIONS = ['low', 'medium', 'high'];
const CONFIDENCE_PRESENTATION = Object.freeze({
  low: { label: 'Low confidence', tone: 'danger', icon: '◔', color: '#FF8A8A' },
  medium: { label: 'Medium confidence', tone: 'warning', icon: '◑', color: '#FFD27A' },
  high: { label: 'High confidence', tone: 'positive', icon: '◕', color: '#8EF0B0' },
});
...
const normalizeTimelineWithProvenance = (events = []) => (Array.isArray(events) ? events : []).map((event) => ({ ...event, provenance: normalizeProvenance(event?.provenance) }));
const normalizeContentWithProvenance = (value = {}) => ({
  ...
  timelineEvents: normalizeTimelineWithProvenance(value?.timelineEvents),
  evidenceLedger: normalizeEvidenceLedger(value?.evidenceLedger, value?.issueNum),
});
const getConfidenceMeta = (provenance) => CONFIDENCE_PRESENTATION[normalizeProvenance(provenance).confidence] || CONFIDENCE_PRESENTATION.medium;
...
const OVERRIDE_REASON_CODES = ['SOURCE_CONFIRMED', 'DOMAIN_EXCEPTION', 'TIME_SENSITIVE', 'FORMAT_PRESERVE'];

const createOverrideAuditEntry = ({ reasonCode = '', warningSource = '' } = {}) => ({
  reasonCode,
  warningSource,
  at: reasonCode ? new Date().toISOString() : '',
});

const compareSemver = (left = '0.0.0', right = '0.0.0') => {
  const [la = 0, lb = 0, lc = 0] = String(left).split('.').map((part) => Number(part) || 0);
  const [ra = 0, rb = 0, rc = 0] = String(right).split('.').map((part) => Number(part) || 0);
  if (la !== ra) return la - ra;
  if (lb !== rb) return lb - rb;
  return lc - rc;
};
const buildRestoreChecksum = (payload = null) => {
  const raw = JSON.stringify(payload ?? {});
  let hash = 0;
  for (let idx = 0; idx < raw.length; idx += 1) hash = ((hash << 5) - hash + raw.charCodeAt(idx)) | 0;
  return `hash32:${Math.abs(hash).toString(16)}`;
};
const buildRestoreDiagnosticsMessage = (diagnostics = []) => diagnostics.map((entry, idx) => `${idx + 1}. ${entry}`).join('\n');
const runRestorePreflight = (input = {}) => {
  const diagnostics = [];
  const schemaVersion = String(input?.schemaVersion || '');
  const appVersion = String(input?.appVersion || '');
  const featureFlags = input?.featureFlags;
  const payload = input?.payload || input;
  const integrity = input?.integrity || {};
  if (!schemaVersion || !appVersion || !featureFlags) diagnostics.push('Missing required backup metadata fields: schemaVersion, appVersion, and featureFlags.');
  if (schemaVersion && compareSemver(schemaVersion, BACKUP_SCHEMA_MIN_SUPPORTED) < 0) diagnostics.push(`Backup schema ${schemaVersion} is older than the supported minimum ${BACKUP_SCHEMA_MIN_SUPPORTED}. Please upgrade this backup in an older app build first, then re-export.`);
  if (schemaVersion && compareSemver(schemaVersion, BACKUP_SCHEMA_MAX_SUPPORTED) > 0) diagnostics.push(`Backup schema ${schemaVersion} is newer than supported max ${BACKUP_SCHEMA_MAX_SUPPORTED}. Upgrade this editor build before restore.`);
  const hasSignature = typeof integrity?.signature === 'string' && integrity.signature.trim().length > 0;
  const hasChecksum = typeof integrity?.checksum === 'string' && integrity.checksum.trim().length > 0;
  if (!hasSignature && !hasChecksum) diagnostics.push('Backup integrity metadata missing. Provide either integrity.checksum or integrity.signature.');
  if (hasChecksum && integrity.checksum !== buildRestoreChecksum(payload)) diagnostics.push('Backup checksum validation failed. File may be corrupted or tampered.');
  if (schemaVersion) {
    const migrationKey = `${schemaVersion}->${BACKUP_SCHEMA_VERSION}`;
    if (!BACKUP_MIGRATION_PATHS[migrationKey] && schemaVersion !== BACKUP_SCHEMA_VERSION) diagnostics.push(`No migration path available for ${migrationKey}.`);
  }
  return { ok: diagnostics.length === 0, payload, diagnostics };
};
...
export default function Editor({ activeFontPairing,showToast,activeTheme,setActiveTheme,editingFrame,clearEditingFrame,newFrameSignal,isActive}){
  ...
  const [saveTagSuggestions, setSaveTagSuggestions] = useState([]);
  ...
  const [previewRedactionEnabled, setPreviewRedactionEnabled] = useLocalStorage('whiz-preview-redaction-enabled', false);
  const [exportRedactionPreset, setExportRedactionPreset] = useLocalStorage('whiz-export-redaction-preset', 'off');
  ...
  const currentDatasetSnapshot = useMemo(() => createDatasetSnapshot({ frameId, theme, content, overrides, aspectRatio, bgGradient, patternOverlay }), [frameId, theme, content, overrides, aspectRatio, bgGradient, patternOverlay]);
  const isSnapshotLockCurrent = Boolean(
    datasetSnapshotRecord
    && datasetSnapshotRecord.hash === currentDatasetSnapshot.hash
    && datasetSnapshotRecord.schemaVersion === currentDatasetSnapshot.schemaVersion
  );
  const refreshSnapshotLock = useCallback(() => {
    setDatasetSnapshotRecord({
      snapshotId: currentDatasetSnapshot.snapshotId,
      hash: currentDatasetSnapshot.hash,
      schemaVersion: currentDatasetSnapshot.schemaVersion,
      savedAt: Date.now(),
    });
    showToast('Snapshot lock refreshed.', 'info');
  }, [currentDatasetSnapshot, setDatasetSnapshotRecord, showToast]);
  const frameRef=useRef(null);const centerRef=useRef(null);
  const redactedPreviewContent = useMemo(
    () => (previewRedactionEnabled ? applyRedactionPreset(content, 'previewSafe') : content),
    [content, previewRedactionEnabled],
  );
  const getExportContent = useCallback((normalizedContent = {}) => {
    const merged = { ...content, ...normalizedContent };
    return applyRedactionPreset(merged, exportRedactionPreset);
  }, [content, exportRedactionPreset]);
  ...
  const doSave=()=>{const snapshot=buildSave();const n=saveName.trim()||`${content.topicTag} \u2014 ${new Date().toLocaleDateString()}`;const normalizedTags=normalizeProtocolTags(saveTagsInput.split(',').map((t)=>t.trim()).filter(Boolean));setSaves(p=>[...p,{id:`s_${Date.now()}`,title:n,...snapshot,tags:normalizedTags.tags,auditTrailSnapshot:[...auditTrail]}]);appendAuditEvent({action:'save',section:'editor',fieldPath:'save',oldValueHash:'n/a',newValueHash:hashValue(snapshot),context:{saveName:n,saveCount:saves.length+1}});setShowSaveModal(false);setSaveName('');showToast(`Saved "${n}"`);};
  useEffect(()=>{setSaveTagSuggestions(extractProtocolTagSuggestions({title:content.title,body:content.body,tableRows:content.tableRows||[]}).slice(0,5));},[content.title,content.body,content.tableRows]);
  ...
  const runPublishExportPreflight=(entrypointLabel,{strict=strictMode,action='export'}={})=>{const requiresSignoff=action==='publish'&&(collaborationMode==='collaborative'||requireSignoffInSolo);const result=runTrustPreflightOrchestrator({content,strictMode:strict,requireSignoff:requiresSignoff,signoffRecord,reviewState});setPreflightResult(result);if(result.blocking.length||result.warnings.length)setTrustPreflightModal({entrypointLabel,result});if(result.blocking.length&&strict){showToast(`Preflight blocked ${entrypointLabel}: ${result.blocking[0].message}`,'error');return{ok:false,result};}return{ok:true,result};};
  ...
  const exportJSON=()=>{if(!validateExportTypography())return;if(!validateCriticalExportFields('json').valid)return;if(!ensureActionAllowed('export'))return;const normalized=runNormalizationPreflight();if(!normalized)return;const payload={...buildSave(),content:{...content,...normalized}};const envelope={schemaVersion:BACKUP_SCHEMA_VERSION,appVersion:'editor-local',featureFlags:BACKUP_FEATURE_FLAGS,payload,integrity:{algorithm:'hash32',checksum:buildRestoreChecksum(payload)}};const d=JSON.stringify(envelope,null,2);const b=new Blob([d],{type:'application/json'});withManagedDownload(b,`${normalized.slug||'whiz_export'}.json`);showToast('JSON exported');};
  ...
  const importJSON=e=>{const f=e.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=ev=>{try{const parsed=JSON.parse(ev.target.result);const preflight=runRestorePreflight(parsed);if(!preflight.ok){const upgradeGuidance=`Restore preflight failed.\n${buildRestoreDiagnosticsMessage(preflight.diagnostics)}\n\nDowngrade guidance: use an editor build that supports schema range ${BACKUP_SCHEMA_MIN_SUPPORTED} - ${BACKUP_SCHEMA_MAX_SUPPORTED} and re-export.\nUpgrade guidance: update this app and retry restore.`;console.warn('restore preflight failed',{diagnostics:preflight.diagnostics,parsed});showToast('Restore blocked: run diagnostics in console.', 'error');window.alert(upgradeGuidance);return;}const d=parseImportedState(preflight.payload);d.frameId&&setFrameId(d.frameId);d.theme&&(setTheme(d.theme),setActiveTheme(d.theme));d.content&&resetContent(normalizeContentWithProvenance(d.content));d.overrides&&setOverrides(d.overrides);d.aspectRatio&&setAspectRatio(d.aspectRatio);d.bgGradient&&setBgGradient(d.bgGradient);d.patternOverlay&&setPatternOverlay(d.patternOverlay);setWorkflowPhase(WORKFLOW_PHASES.includes(d.workflowPhase)?d.workflowPhase:'draft');setPhaseChecklist(d.phaseChecklist||{draftAt:d.savedAt||Date.now(),reviewAt:null,publishReadyAt:null,lastTransitionAt:d.savedAt||Date.now()});showToast('Imported');}catch(err){showToast('Invalid JSON','error');}};r.readAsText(f);e.target.value='';};
  ...
  const exportWebP=async()=>{if(!validateExportTypography())return;
    ...
    try{setDatasetSnapshotRecord({snapshotId:currentDatasetSnapshot.snapshotId,hash:currentDatasetSnapshot.hash,schemaVersion:currentDatasetSnapshot.schemaVersion,savedAt:Date.now()});
      const sceneModel=createSceneModel({frameId,theme,content:getExportContent(normalizedContent),overrides,aspectRatio,bgGradient});
      ...
      await new Promise((res,rej)=>cv.toBlob(b=>{
        ...
        const objectUrl=URL.createObjectURL(b);const a=document.createElement('a');
        const baseName=normalizedContent.slug||'whiz_export';const imageFileName=`${baseName}.webp`;try{a.href=objectUrl;a.download=imageFileName;a.click();}finally{setTimeout(()=>URL.revokeObjectURL(objectUrl),0);} 
      ...
      <div className={`editor-right ${mobileTab==='content'?'mob-active':''}`}>
        ...
        {rightTab==='design'?(<><div className={`edit-toggle-row ${editMode?'on':''}`} onClick={()=>{setEditMode(m=>!m);editMode&&setSelectedEl(null);}}><div className={`toggle-pill ${editMode?'on':''}`}><div className="toggle-dot"/></div><span className="toggle-label">{editMode?'Edit ON':'Edit OFF'}</span><span className="toggle-hint">{editMode?'Click elements':'Toggle to edit'}</span></div><DesignPanel selectedEl={selectedEl} setSelectedEl={setSelectedEl} overrides={overrides} setOverrides={setOverrides} theme={theme} bgGradient={bgGradient} setBgGradient={setBgGradient} showToast={showToast} resetOverrides={resetOverrides} setPatternOverlay={setPatternOverlay} strictMode={strictMode}/>
          ...
          <div className="editor-section">
            ...
            <div style={{marginTop:8,fontFamily:'var(--font-m)',fontSize:10,color:correctionCount>0?'#E5B23A':'var(--dim)'}}>
