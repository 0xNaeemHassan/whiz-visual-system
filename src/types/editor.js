/** @typedef {{ id:number, tier:string, tierName:string, name:string, desc:string, tags:string[], layout:string }} FrameDefinition */
/** @typedef {{ label:string, value:string }} StatItem */
/** @typedef {{ amount?:number, type?:'fee'|'benefit'|'risk', bps?:number, feeUsd?:number, [key:string]: string|number|undefined }} TableRow */
/** @typedef {{ issueNum:string, date:string, desk:string, volume:string, topicTag:string, title:string, deck:string, body:string, handle:string, socialX?:string, socialSub?:string, status?:string, tickerSpeed?:number, sparkData?:string, stats:StatItem[], tableRows:TableRow[], tableHeaders:string[], bullPoints:string[], bearPoints:string[], bigNumber?:string, bigLabel?:string, verdict?:string, gridItems?:unknown[], timelineEvents?:unknown[], [key:string]: unknown }} FrameContent */
/** @typedef {{ fontSize?:number, fontWeight?:number, color?:string|null, italic?:boolean, lineHeight?:number, letterSpacing?:number, textAlign?:'left'|'center'|'right', opacity?:number, background?:string|null, borderColor?:string|null }} TextStyleOverride */
/** @typedef {{ frameBg?:string|null, spineColor?:string|null, tickerColor?:string|null, tickerBg?:string|null, title?:TextStyleOverride, deck?:TextStyleOverride, body?:TextStyleOverride, accent?:{color?:string|null}, tag?:TextStyleOverride, footer?:TextStyleOverride, statsColor?:string|null, bignumColor?:string|null, avatarColor?:string|null, ruleBg?:string|null, handleColor?:string|null, [key:string]: unknown }} StyleOverrides */
/** @typedef {{ id:string, name:string, accent:string, base:string, [key:string]: unknown }} Theme */
/** @typedef {{ frameId:number, frame:FrameDefinition, theme:Theme, content:FrameContent, overrides:StyleOverrides, aspectRatio?:{id?:string,w:number,h:number}, bgGradient?:string|null, patternOverlay?:{id?:string, css:string, size?:string, opacity?:number}|null, savedAt?:number|string }} ExportPayload */

export {};
